function fig = launch_task4_demo(visible)
%LAUNCH_TASK4_DEMO 任务4三模块面板：控制台(主) + 飞机模型 + 环境模型。
...
fig=uifigure('Name','任务4正弦风程序：控制台×飞机模型×环境模型(动态演示)',...
...
header=uilabel(outer,'Text','任务4程序 | environment 正弦风模型 × aircraft 双表盘黑箱 × console 寻优算法',...
    'FontName','Microsoft YaHei','FontSize',16,'FontWeight','bold'); put(header,1,[1 2]);
left=uigridlayout(outer,[5 1]); put(left,2,1);
left.RowHeight={38,'1x',150,30,'1x'};
...
g=uigridlayout(form,[19 2]); g.ColumnWidth={140,'1x'};
g.RowHeight=[repmat({30},1,15),{26,30,26,30}];
...
windOmega=number(g,'风角频率ω / rad·s⁻¹',13,0.08,[0 2]);
windBias=number(g,'风偏置B / m·s⁻¹',14,3.0,[0 10]);
circleT=number(g,'盘旋周期 / s',15,80,[10 400]);
truth=uicheckbox(g,'Text','显示评价器曲线与真值','Value',true); put(truth,16,[1 2]);
energy=uicheckbox(g,'Text','搜索能耗计入评价(续航口径)','Value',true); put(energy,17,[1 2]);
speedLabel=uilabel(g,'Text','播放速度: 1.00x','FontName','Microsoft YaHei','FontSize',11); put(speedLabel,18,1);
speedSlider=uislider(g,'Limits',[0.5 8],'Value',1,'MajorTicks',[.5 1 2 4 8],...
    'MajorTickLabels',{'0.5x','1x','2x','4x','8x'}); put(speedSlider,18,2);
...
readout=uilabel(left,'Text','','WordWrap','on','FontName','Microsoft YaHei'); put(readout,4,1);
logBox=uitextarea(left,'Editable','off','FontName','Microsoft YaHei','FontSize',9); put(logBox,5,1);
...
controls=struct('algorithm',algorithm,'scenario',scenarioC,'initial',initial,...
    ...
    'windAmp',windAmp,'windOmega',windOmega,'windBias',windBias,'circleT',circleT,'truth',truth,'energy',energy,...
...
envPanel=uipanel(plots,'Title','环境模型：正弦风×圆周航迹','FontName','Microsoft YaHei',...
...
ud=fig.UserData; ud.airPanel=airPanel; ud.envPanel=envPanel; fig.UserData=ud;
% 环境模型静态要素
...
hWind=line(axEnv,nan,nan,'Color',[.0 .55 .25],'LineWidth',2.4,'DisplayName','风');
hHome=line(axEnv,nan,nan,'Color',[.4 .4 .4],'Marker','s','MarkerSize',8,...
    'LineStyle','none','DisplayName','盘旋中心');
hEnvTxt1=text(axEnv,0,0,'','FontName','Microsoft YaHei','FontSize',9,'Color',[.0 .4 .2]);
...
hWindTip=text(axEnv,0,0,char(10132),'FontName','Segoe UI Symbol','FontSize',13,...
...
% 风速变化曲线(任务4): W(t)=A sin(w t)+B
hWindCurve=line(axWind,nan,nan,'Color',[.0 .55 .25],'LineWidth',1.2,'DisplayName','风');
hWindNow=line(axWind,nan,nan,'Color',[.85 .18 .18],'Marker','o','MarkerFaceColor',...
    [.85 .18 .18],'MarkerSize',5,'LineStyle','none','DisplayName','当前风速');
...
for cn={algorithm,scenarioC,initial,noise,ripA1,ripL1,ripA2,ripL2,shiftTime,shiftDx,seed,windAmp,windOmega,windBias,circleT}
...
logMsg('任务4正弦风程序就绪 | console + aircraft + environment | 风速 W(t)=A·sin(ωt)+B');
logMsg('提示: 左侧三按钮切换界面; 小窗口内"放大⤢"可放大/还原; 风速=0即无风任务1+2口径');
if exist(fullfile(root,'results','report.md'),'file')
    loadReport();
else
    logMsg('尚未生成验收报告: 命令行运行 run_task4_modules_acceptance 后点"载入验收报告"');
end
prepare();
    ...
    function updateModules(k)
        ...
        Wnow=c.windAmp*sin(c.windOmega*tNow)+c.windBias;   % 当前风速(带符号)
        WL=R*(0.16+0.055*abs(Wnow));                % 小箭头长度随风速大小
        arrowDir=windDir + (Wnow<0)*180;            % 负风速=反向
        tipx=WL*cosd(arrowDir); tipy=WL*sind(arrowDir);
        hWind.XData=[0.3*tipx, 0.68*tipx]; hWind.YData=[0.3*tipy, 0.68*tipy];
        hWindTip.Position=[tipx,tipy,0];
        hWindTip.Rotation=arrowDir;                 % ➒字符小箭头, 随风向旋转
        hasW=abs(Wnow)>1e-9;
        hWind.Visible=hasW; hWindTip.Visible=hasW;
        hEnvTxt1.HorizontalAlignment='center';
        hEnvTxt2.HorizontalAlignment='center';
        if hasW
            hEnvTxt1.Position=[0, -R*0.18, 0];
            hEnvTxt1.String=sprintf('风向 %d° | 风速 %+.2f m/s',windDir,Wnow);
        else
            hEnvTxt1.String='';
        end
        ...
        axis(axEnv,'equal');
        xlim(axEnv,[-R*1.15 R*1.15]); ylim(axEnv,[-R*1.25 R*1.28]);
        xlabel(axEnv,'x / m'); ylabel(axEnv,'y / m');
        % 风速变化曲线
        tt=linspace(0,c.duration*c.tEval,600);
        WW=c.windAmp*sin(c.windOmega*tt)+c.windBias;
        hWindCurve.XData=tt; hWindCurve.YData=WW;
        hWindNow.XData=tNow; hWindNow.YData=Wnow;
        hWindZero.XData=[0 tt(end)]; hWindZero.YData=[0 0];
        xlim(axWind,[0 tt(end)]);
        ylim(axWind,[min(WW)-0.6,max(WW)+0.6]);
    end

    function resizeLayout(varargin)
        bodyHeight=max(440,fig.InnerPosition(4)-112);
        outer.RowHeight={44,bodyHeight,28};
        left.RowHeight={38,max(240,bodyHeight-430),150,30,max(160,bodyHeight-460)};
    end
    ...
    function prepare(varargin)
        ...
        try
            ...
            logMsg(sprintf(['重置完成: 算法=%s 场景=%s 初速=%.1f 噪声=%.3f '...
                ...
                '| 风: A=%.1f ω=%.2f B=%.1f 盘旋周期=%.0f s 半径=%d m'],...
                ...
                windAmp.Value,windOmega.Value,windBias.Value,circleT.Value,c.circleRadius));
        catch err
    ...
    function loadReport(varargin)
        ...
        if ~exist(rp,'file')
            logMsg('未找到验收报告: 请先在命令行运行 run_task4_modules_acceptance');
            return;
        end
        lines=readlines(rp);
