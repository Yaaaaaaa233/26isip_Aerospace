function summary = run_task4_modules_acceptance(varargin)
%RUN_TASK3_ACCEPTANCE 任务4验收：正弦风W(t)=A·sin(ωt)+B×圆周盘旋×崎岖对象×MOP/MOE。
%
% 用法: run_task4_modules_acceptance / summary=run_task4_modules_acceptance(...)
...
cRef=wsearch.config('windAmp',2,'windOmega',0.08,'windBias',3,'circlePeriod',80);  %#ok<NASGU> 默认风场口径
folder=fullfile(root,'results'); if ~exist(folder,'dir'), mkdir(folder); end
unit=runtests(fullfile(root,'tests_task4_modules.m'));
...
for name=algorithms
    for seed=11:30
        c=wsearch.config(varargin{:},'seed',seed,'windAmp',2,'windOmega',0.08,'windBias',3,'circlePeriod',80);
        if strcmp(name{1},'tracker'), c=wsearch.config(varargin{:},'seed',seed,...
            'windAmp',2,'windOmega',0.08,'windBias',3,'circlePeriod',80,'rippleA1',0,'rippleA2',0,'noiseSigma',0); end
    ...
    for seed=11:20
        for name={'ea_multistart','tracker'}
            ...
            if strcmp(name{1},'tracker'), c=wsearch.config(varargin{:},...
                'seed',seed,'windAmp',2,'windOmega',0.08,'windBias',3,'circlePeriod',80,...
...
for seed=11:15
    c=wsearch.config(varargin{:},'seed',seed,'energyAccounting',false,...
        'windAmp',2,'windOmega',0.08,'windBias',3,'circlePeriod',80);
    scn=wsearch.scenario('static',c);
...
for name={'ea_multistart','tracker','esc'}
    for seed=11:13
        c=wsearch.config(varargin{:},'seed',seed,'T',3600,'duration',3600,...
            'tailSteps',300,'windAmp',2,'windOmega',0.08,'windBias',3,'circlePeriod',80);
        scn=wsearch.scenario('static',c);
        [log,~]=wsearch.run_algorithm(name{1},scn,c);
        m=wsearch.mop_moe(log,c);
...
checks=[...
    ...
    struct('item','ea 风场静态尾段跟踪误差(omitnan) <=0.5 且 >=18/20 种子',...
        'pass',sum(eaTail<=0.5)>=18),...
...
fprintf(fid,'# 任务4验收：正弦风W(t)=A·sin(ωt)+B×圆周盘旋×崎岖对象×MOP/MOE\n\n生成时间：%s\n\n',...
    datestr(now,31));
