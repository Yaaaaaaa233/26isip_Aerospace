...
drift=false;
f=usearch.search_query(plant);
s0=usearch.brent_search(f,p.lower,p.upper,p.tol,min(p.maxSearchEval,n-1));
appendBrackets(s0);
...
while plant.count()<n
    ...
    for attempt=1:p.t1Retry+1
        ...
        f=usearch.search_query(plant);
        sr=usearch.brent_search(f,a,b,p.tol,min(p.maxSearchEval,n-plant.count()));
        appendBrackets(sr);
        edge=0.05*(b-a);
