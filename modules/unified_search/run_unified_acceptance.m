...
% Z6 (reacceptance F5): the acceptance must fail hard when any unit test
% or performance gate is missed -- automation must receive a non-success
% result, not a warning.
summary.passed = (summary.gatesPassed == summary.gatesTotal) && ...
    (summary.unitPassed == summary.unitTotal);
if ~summary.passed
    error('usearch:AcceptanceFailed', ...
        'unified acceptance failed: units %d/%d, gates %d/%d (see %s)', ...
        summary.unitPassed, summary.unitTotal, ...
        summary.gatesPassed, summary.gatesTotal, folder);
end
end
