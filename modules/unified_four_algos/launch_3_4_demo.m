function fig = launch_3_4_demo(visible)
%LAUNCH_3_3_DEMO 任务3.4标定+风推断混合面板：控制台(主) + 飞机模型 + 环境模型。
...
% —— 自愈暂存(2026-09-08): 绕开MATLAB目录缓存滞后导致的 w34.* 解析失败 ——
if exist('w34.fit_curve_wind','file')~=2
    stg=fullfile(tempdir,['w34stage_' datestr(now,'yyyymmddHHMMSS') '_' num2str(randi(8999)+1000)]);
    copyfile(fullfile(root,'+w34'),fullfile(stg,'+w34'));
    addpath(stg); clear functions; rehash;
end
fig=uifigure('Name','任务3.4风速推断寻优：空速=地速−风速(顺风右移/逆风左移) × 风不影响运动 × 七种风场(动态演示)',...
...
header=uilabel(outer,'Text','任务3.4程序 | 空速=地速−风速(地速6+顺风3→空速3) × 曲线=空速曲线(固定) × 地速曲线顺风右移/逆风左移 × 曲线未知(调速→功率黑盒) × 首飞3→12全速域快扫标定 × MOE=纯能耗(Emin/Eactual)',...
...
algorithm=choice(g,'控制策略',1,{'sweepcal全速域标定+精化','hybrid标定+风推断混合',...
    'rl仿真器预训练+微调','purerl纯奖励RL(免标定)','openloop开环基线',...
    'windinfer已知曲线oracle参照','est已知曲线EKF(参照)','known已知风+曲线oracle上限'},...
    {'sweepcal','hybrid','rl','purerl','openloop','windinfer','est','known'},'sweepcal');
...
logMsg('任务3.4标定+风推断混合程序就绪 | 主角hybrid: 首飞3→12双向扫标定(曲线冻结)→在线windinfer式风推断+闭式调度 | 对照: sweepcal/rl/openloop + windinfer/est/known oracle参照');
...
prepare();
    ...
    function updateModules(k)
        ...
        [Wx,Wy,Vx,Vy]=w34.wind_field(scn,tNow,psi);
        ...
        if strcmp(c.windKind,'sector')   % 扇区风: 横轴=航向, 当前点=此刻航向处的风
            ...
            [WxC,WyC]=w34.wind_field(scn,0,deg2rad(psd));
            ...
            [WxC,WyC]=w34.wind_field(scn,tt,0);
    ...
    function buildConfig()
        c=w34.config('initialSpeed',initial.Value,'noiseSigma',noise.Value,'curveCase',curveC.Value,...
    ...
    function prepare(varargin)
        ...
        try
            ...
            if strcmp(algorithm.Value,'purerl') && isstruct(info) && isfield(info,'muB')
                logMsg(sprintf(['纯奖励RL(免标定): 无扫频/无模型, 大幅值对偶探索(σ=%.1f→%.1f)+邻域核+分桶基线 | ',...
                    'μ轮廓幅值=%.2f m/s | 终态σ=%.2f'],...
    ...
    function windPreview()
        ...
        if strcmp(c.windKind,'sector')
            ...
            [WxP,WyP]=w34.wind_field(pvScn,tt,0);
    ...
    function drawCasePreview()
        ...
        for q=1:3
            cc=w34.config(c,'curveCase',cv(q));
            hds(q).XData=vv; hds(q).YData=w34.base_curve(vv,cc)*c.pHover;
    ...
    function redraw()
        ...
        [WxK,WyK,VxK,VyK]=w34.wind_field(scn,tNow,psiK);
        vv=linspace(c.lower,c.upper,400);
        if vis
            uu=hypot(vv*cos(psiK)-VxK,vv*sin(psiK)-VyK);
            h.curve.XData=vv; h.curve.YData=(w34.base_curve(uu-L.shiftDx(k),c)+L.shiftDy(k))*c.pHover;
            h.curveAir.XData=vv; h.curveAir.YData=(w34.base_curve(vv-L.shiftDx(k),c)+L.shiftDy(k))*c.pHover;
