function summary = run_task34_checks()
%RUN_TASK31_CHECKS 任务3.4检查：曲线未知(黑盒调速→功率) + 全速度域标定寻优 + RL对照。
root=fileparts(mfilename('fullpath')); addpath(root);
folder=fullfile(root,'results'); if ~exist(folder,'dir'), mkdir(folder); end
unit=runtests(fullfile(root,'tests_task34.m'));
...
for kk=1:numel(kinds)
    for name=allPol
        ...
        for i=1:2
            c=w34.config('seed',10+i,'duration',250,'tailSteps',5,'windKind',kinds{kk},...
                'windAmpY',1.5,'windBiasY',1);
            [log,~]=w34.run_algorithm(name{1},w34.scenario('static',c),c);
            ex(i)=w34.mop_moe(log,c).energyExcessPercent;
...
for iw=1:size(windKinds,1)
    for name=policies
        ...
        for i=1:2
            c=w34.config('seed',2+i,'duration',800,'tailSteps',60,windKinds{iw,2}{:});
            scn=w34.scenario('static',c);
            [log,info]=w34.run_algorithm(name{1},scn,c);
            m=w34.mop_moe(log,c);
...
% ---- 物理口径核验 ----
c=w34.config('seed',11,'duration',30,'tailSteps',5,'windKind','sin',...
...
checks=[...
    struct('item','单元测试全绿(四主角sweepcal/rl/purerl/hybrid+风场库/空速语义/执行链)','pass',sum([unit.Passed])==numel(unit)),...
    ...
    struct('item','恒定风: purerl 优于开环 且 超额<5%','pass',pcOK),...
...
fprintf(fid,['# 任务3.4检查：四算法整合(sweepcal/rl/purerl/hybrid) 同对象同口径横比\n\n生成时间：%s\n\n'],datestr(now,31));
...
fprintf('检查门槛：%d/%d\n',summary.gatesPassed,summary.gatesTotal);
if summary.gatesPassed<summary.gatesTotal, warning('w34:Checks','Some gates missed.'); end
end
