%TABLE_3X3 任务3.4 三乘三验证表：飞机模式 × 风况, 单元格=MOE(纯能耗 Emin/Eactual)。
% 飞机模式: 悬停(v=0) / 匀速转圈(地速≡名义6.3) / 变速转圈(sweepcal, 曲线未知)。
% 风况: 无风 / 恒定风(3.5 m/s @40°) / 变风(composite)。
% 任务3.4特有口径: 控制器不知道曲线(u*未知)也不知道风——匀速行飞名义6.3是
...
for iw=1:3
    for im=1:3
        vals=[];
        for sd=[3 6 9]
            c=w34.config('seed',sd,'duration',800,'tailSteps',60,windCfg{iw}{:});
            scn=w34.scenario('static',c);
            if im==3
                [log,~]=w34.run_algorithm('sweepcal',scn,c);
            else
                if im==1, olv=0; else, olv=6.3; end
                c2=w34.config(c,'openLoopV',olv);
                [log,~]=w34.run_algorithm('openloop',scn,c2);
            end
    ...
    for sd=[3 6 9]
        c=w34.config('seed',sd,'duration',800,'tailSteps',60,windCfg{iw}{:});
        [log,~]=w34.run_algorithm('known',w34.scenario('static',c),c);
