...
while plant.count()<n
    ...
    g=probeSlope(bestV);
    % 大幅越限(>8倍阈值, 双风叠加或错谷锁定的特征): 宽幅重定位直接跳谷,
    % 避免5点stencil每次只挪0.25而永远爬不回正确谷底
    wide=abs(muNow-refP) > 8*lvlThr;
    bestV=recenter(bestV,wide);
...
info.best=bestV;
    ...
    function cOut=recenter(cIn,wide)
        % 轻量重定位: 单次5点对称stencil+最小二乘顶点(clamp ±0.25)。
        % 对称设计在静止对象上顶点≈当前点(零净漂移), 在缓变/周期偏移下
        % 每次向新最优靠近一步; 不用argmin局部搜索(纹波斜率会致随机游走)。
        % wide=true(大幅越限): 9点宽域扫描(±4 m/s)直接跳到最低点——错谷
        % 锁定时的快速恢复路径, 成本仅9评估。
        cOut=cIn;
        if plant.count()>n-6, return; end
        if nargin>=2 && wide && plant.count()<=n-9
            vw=linspace(max(p.lower,cIn-4),min(p.upper,cIn+4),9);
            pw=Inf(1,9);
            for i=1:9
                if plant.count()>=n, break; end
                pw(i)=plant.q(vw(i),'probe');
            end
            [~,iw]=min(pw);
            if isfinite(pw(iw))
                cOut=vw(iw); plant.amendEstimate(cOut);
                info.relocals=info.relocals+1; info.phase='local';
                return;
            end
        end
    ...
    function [vOut,pOut]=globalEscalate(cSeed)
        % 任务5: 奇偶交错两段采样+各段均值去趋势——顺序扫描会横跨半个
        % 风周期, 时变能级(dv)把滤波 argmin 倾斜到错误谷; 交错后每段的
        % 慢漂移近似常数, 减去段均值即消除总倾斜(总评估数不变)。
        vs=linspace(p.lower,p.upper,p.eaScanN);
        pr=Inf(1,p.eaScanN);
        odd=find(mod(1:p.eaScanN,2)==1); even=find(mod(1:p.eaScanN,2)==0);
        for i=[odd,even]
            if plant.count()>=n, break; end
            pr(i)=plant.q(vs(i),'scan');
        end
        mOdd=mean(pr(odd),'omitnan'); mEven=mean(pr(even),'omitnan');
        pr(odd)=pr(odd)-mOdd; pr(even)=pr(even)-mEven;
