function fig = launch_task5_demo(visible)
%LAUNCH_TASK5_DEMO 任务5三模块面板：控制台(主) + 飞机模型 + 环境模型。
...
fig=uifigure('Name','任务5双正交风程序：控制台×飞机模型×环境模型(动态演示)',...
...
header=uilabel(outer,'Text','任务5程序 | environment 双正交正弦风模型 × aircraft 双表盘黑箱 × console 寻优算法',...
...
logMsg('任务5双正交风程序就绪 | x向: A·sin(ω1t)+B, y向: C·sin(ω2t)+D | 合成矢量见环境模型');
logMsg('提示: 左侧三按钮切换界面; 小窗口内"放大⤢"可放大/还原; 风速=0即无风任务1+2口径');
if exist(fullfile(root,'results','report.md'),'file')
    loadReport();
else
    logMsg('尚未生成验收报告: 命令行运行 run_task5_modules_acceptance 后点"载入验收报告"');
end
prepare();
    ...
    function loadReport(varargin)
        ...
        if ~exist(rp,'file')
            logMsg('未找到验收报告: 请先在命令行运行 run_task5_modules_acceptance');
            return;
        end
        lines=readlines(rp);
