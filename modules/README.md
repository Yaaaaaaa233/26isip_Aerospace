...
| [`platform_link`](platform_link/) | 第三轮3.6：算法×平台对接（三原语q/amendEstimate/count接平台P2物理链；预算按秒、T1测量链0.2s延迟+1.2%噪声、评价真值取harness适配器权威源、功率悬停归一）；⚠️接手修复中（2026-09-09 叶安接手：q()时间单位错位、Phase B风修正无界发散、R2022b环境探测等缺陷，方案见 [`T36_PLATFORM_LINK_TAKEOVER_PLAN`](../docs/T36_PLATFORM_LINK_TAKEOVER_PLAN.md)，原WIP交接文档在模块目录） | `launch_3_6_demo.m`、`run_task36_checks.m` | [`docs/evidence/platform_link/`](../docs/evidence/platform_link/) | 王健祺（原创）/ 叶安（接手修复） |
