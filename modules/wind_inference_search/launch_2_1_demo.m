function fig = launch_2_1_demo(visible)
%LAUNCH_2_1_DEMO 任务2.1风速推断寻优面板：控制台(主) + 飞机模型 + 环境模型。
...
fig=uifigure('Name','任务2.1风速推断寻优：空速=地速−风速(顺风右移/逆风左移) × 风不影响运动 × 七种风场(动态演示)',...
...
header=uilabel(outer,'Text','任务2.1程序 | 空速=地速−风速(地速6+顺风3→空速3) × 曲线=空速曲线(固定) × 地速曲线顺风右移/逆风左移 × MOE=纯能耗(Emin/Eactual)',...
...
phaseMap={'local','局部';'sigma','噪声估计';'far','远点证据';'scan','扫描';...
    ...
    'est','估计跟踪';'infer','风推断';'oracle','oracle'};
...
logMsg('任务2.1风场模型库程序就绪 | 七种风场: 恒定/双正弦/方波(软边)/三角/湍流(OU)/复合(推荐)/扇区(随航向) | 下拉选中即预览');
...
if exist(fullfile(root,'results','report.md'),'file')
    ...
    logMsg('尚未生成验收报告: 命令行运行 run_task21_checks 后点"载入验收报告"');
end
prepare();
    ...
    function updateModules(k)
        ...
        [Wx,Wy,Vx,Vy]=w21.wind_field(scn,tNow,psi);
        ...
        if strcmp(c.windKind,'sector')   % 扇区风: 横轴=航向, 当前点=此刻航向处的风
            ...
            [WxC,WyC]=w21.wind_field(scn,0,deg2rad(psd));
            ...
            [WxC,WyC]=w21.wind_field(scn,tt,0);
    ...
    function buildConfig()
        c=w21.config('initialSpeed',initial.Value,'noiseSigma',noise.Value,'curveCase',curveC.Value,...
    ...
    function windPreview()
        ...
        if strcmp(c.windKind,'sector')
            ...
            [WxP,WyP]=w21.wind_field(pvScn,tt,0);
    ...
    function drawCasePreview()
        ...
        for q=1:3
            cc=w21.config(c,'curveCase',cv(q));
            hds(q).XData=vv; hds(q).YData=w21.base_curve(vv,cc)*c.pHover;
    ...
    function redraw()
        ...
        [WxK,WyK,VxK,VyK]=w21.wind_field(scn,tNow,psiK);
        vv=linspace(c.lower,c.upper,400);
        if vis
            uu=hypot(vv*cos(psiK)-VxK,vv*sin(psiK)-VyK);
            h.curve.XData=vv; h.curve.YData=(w21.base_curve(uu-L.shiftDx(k),c)+L.shiftDy(k))*c.pHover;
            h.curveAir.XData=vv; h.curveAir.YData=(w21.base_curve(vv-L.shiftDx(k),c)+L.shiftDy(k))*c.pHover;
    ...
    function loadReport(varargin)
        ...
        if ~exist(rp,'file')
            logMsg('未找到验收报告: 请先在命令行运行 run_task21_checks');
            return;
        end
        lines=readlines(rp);
