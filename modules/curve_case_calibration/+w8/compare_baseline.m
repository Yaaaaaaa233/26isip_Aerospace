...
if ~any(strcmp(name,{'tracker','esc','spsa','bayes','qnewton','gtrack','est'}))
    error('w7:CompareBaseline','Baseline comparison needs an adaptive algorithm, got %s.',name);
end
cB=c; cB.seed=c.seed;
[logA,~]=w8.run_algorithm(name,scn,c);
mA=w8.mop_moe(logA,c);
[logB,~]=w8.run_algorithm('openloop',scn,cB);
mB=w8.mop_moe(logB,cB);
