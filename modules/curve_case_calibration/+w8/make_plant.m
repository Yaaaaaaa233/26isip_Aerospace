...
[J0min, ~] = w8.base_curve(c.optimum0, c);
rng(c.seed);
...
plant = struct('q',@q,'amendEstimate',@amend,'count',@countFcn,'table',@tbl,...
    ...
    function Jm=q(v,tag)
        ...
        for m=1:M
            ...
            % ---- 风功率路径(空速物理): u=|v·t̂+w| ----
            [~,~,Vx,Vy]=w8.wind_components(scn,tNow);
            ux=vAct*cos(psi)+Vx; uy=vAct*sin(psi)+Vy;
            u=hypot(ux,uy);
            [dxS,dyS]=w8.shift_truth(scn,tNow);
            Psub=w8.base_curve(u-dxS,c)+dyS;
        ...
        tEnd=tStep+c.tEval;
        [~,~,VxE,VyE]=w8.wind_components(scn,tEnd);
        qE=VxE*cos(psi)+VyE*sin(psi);
        w2E=VxE^2+VyE^2;
        [dxE,~]=w8.shift_truth(scn,tEnd);
    ...
    function out=truthCurve()
        ...
        out.v=vv; out.J=w8.base_curve(vv,c);
    end
    function out=tbl()
