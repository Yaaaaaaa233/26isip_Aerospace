...
% 所有算法共享同一条指令就位规则(w8.settled_q): 横比只反映策略差异。
% 算法侧白名单：不传场景真值/曲线/噪声/种子(红线1); 接口与任务6一致
% (plant.q(v,tag)), 对象侧升级不改控制器接口(红线2)。
p=c;   % 统一config即白名单(对象真值由plant持有, 不经p传递)
plant=w8.make_plant(scn,c);
n=c.duration;
switch name
    case 'openloop'
        info=w8.openloop_run(plant,p,n);
    case 'tracker'
        info=w8.tracker_run(plant,p,n);
    case 'esc'
        info=w8.esc_run(plant,p,n);
    case 'spsa'
        info=w8.spsa_run(plant,p,n);
    case 'bayes'
        info=w8.bayes_run(plant,p,n);
    case 'qnewton'
        info=w8.qnewton_run(plant,p,n);
    case 'gtrack'
        info=w8.gtrack_run(plant,p,n);
    case 'est'
        info=w8.est_run(plant,p,n);
    case 'known'
        info=w8.known_run(plant,p,n,scn);
    otherwise
