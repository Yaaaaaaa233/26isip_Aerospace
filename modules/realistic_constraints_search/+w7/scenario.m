...
%SCENARIO 平移调度(评价侧, 不给搜索器)。种类与任务6一致, 可与风场叠加：
...
% 任务7差异: 风致偏移不再用固定盘旋周期, 而由对象内积分航向 psi'(=v/R) 驱动
% (见 plant.m), 故 scn 只携带风参数与平移调度, 不再携带 circlePeriod。
t1=c.shiftTime;
switch kind
    ...
    otherwise, error('w7:Scenario','Unknown scenario kind: %s',kind);
end
