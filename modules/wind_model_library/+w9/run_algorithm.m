...
% 所有算法共享同一条指令就位规则(w9.settled_q): 横比只反映策略差异。
...
switch name
    case 'openloop'
        info=w9.openloop_run(plant,p,n);
    case 'tracker'
        info=w9.tracker_run(plant,p,n);
    case 'esc'
        info=w9.esc_run(plant,p,n);
    case 'spsa'
        info=w9.spsa_run(plant,p,n);
    case 'bayes'
        info=w9.bayes_run(plant,p,n);
    case 'qnewton'
        info=w9.qnewton_run(plant,p,n);
    case 'gtrack'
        info=w9.gtrack_run(plant,p,n);
    case 'est'
        info=w9.est_run(plant,p,n);
    case 'known'
        info=w9.known_run(plant,p,n,scn);
    otherwise
        error('w9:RunAlgorithm','Unknown algorithm: %s',name);
end
