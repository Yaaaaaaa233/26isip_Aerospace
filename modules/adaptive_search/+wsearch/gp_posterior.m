...
% bayes_run 的底层拟合器(独立成函数以便单元测试)。
% 输入: Vw,Yw 观测窗口(列向量); vGrid 候选网格(行向量);
%       ell 核长, sF 信号标准差, sN 测量噪声标准差。
% 输出: out.mu, out.sig 候选网格上的后验均值/标准差(列向量)。
...
if flag>0
    ...
    [R,flag]=chol(K);
    assert(flag==0,'gp_posterior:Cholesky','Kernel matrix not PD.');
end
alpha=R\(R'\yc);
