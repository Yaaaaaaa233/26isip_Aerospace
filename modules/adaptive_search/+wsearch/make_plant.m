...
[~, J0min] = wsearch.base_curve(c.optimum0, c);
rng(c.seed);
plant = struct('q',@q,'amendEstimate',@amend,'count',@countFcn,'table',@tbl,...
    ...
    function Jm=q(v,tag)
        assert(k<n,'wsearch:Plant','Evaluation budget exhausted.');
        k=k+1; t=(k-1)*c.tEval;
        [dx,dy]=wsearch.shift_truth(scn,t);
        truth=wsearch.base_curve(v-dx,c)+dy;
    ...
    function out=truthCurve()
        ...
        out.v=vv; out.J=wsearch.base_curve(vv,c);
    end
    function out=tbl()
        assert(k==n,'wsearch:Plant','Run incomplete: %d of %d steps.',k,n);
