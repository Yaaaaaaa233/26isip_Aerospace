...
%SWEEPCAL_RUN 全速度域快扫标定 + 在线精化(任务3.2主角策略, 纯因果, 曲线未知)。
...
while plant.count()<n
    ...
    % ---- Phase A 标定点: 半程预热拟合 + 全程正式拟合 ----
    if kStep==floor(p.swSteps/2) || kStep==p.swSteps
        [coefs,wSm,fitRms,uLo,uHi]=w32.fit_curve_wind(calPsi(1:kStep),...
            calV(1:kStep),calP(1:kStep),p);
        uStar=w32.curve_argmin(coefs,uLo,uHi,p);
        if kStep==p.swSteps
    ...
    if phase==2 && mod(kStep,p.swRefitEvery)==0 && nr>=30
        ...
        [dwC,~]=w32.wind_corr(rnPsi(1:nr),rnV(1:nr),rnP(1:nr),coefs,wSm,p);
        wSm=wSm+0.8*dwC;
    end
    if phase==2
        if mod(kStep,p.swRefitEvery)==0 && nr>=30
            ...
            [coefsN,~,fitRmsN,uLoN,uHiN]=w32.fit_curve_wind(psSet,vSet,pSet,p,wSm);
            sseOld=w32.curve_sse(coefs,wSm,psSet,vSet,pSet);
            sseNew=w32.curve_sse(coefsN,wSm,psSet,vSet,pSet);
            if sseNew < sseOld*0.999
                coefs=coefsN; uLo=uLoN; uHi=uHiN; fitRms=fitRmsN;
            end
            uStar=w32.curve_argmin(coefs,uLo,uHi,p);
        end
        if mod(kp,p.ucProbeEvery)==2 && nr>=2
            ...
            if abs(sPair)>p.ucThr
                ...
                uPrev=uStar;
                uStar=min(max(uStar+du,p.lower+0.5),p.upper-1);
                sPrev=sPair;
            else
