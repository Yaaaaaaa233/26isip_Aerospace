function summary = run_task35_checks()
%RUN_TASK35_CHECKS 任务3.5检查：预训练purerl拆分(在线/离线) + 继承算法横比。
root=fileparts(mfilename('fullpath')); addpath(root);
folder=fullfile(root,'results'); if ~exist(folder,'dir'), mkdir(folder); end
unit=runtests(fullfile(root,'tests_task35.m'));
...
for kk=1:numel(kinds)
    for name=allPol
        ...
        for i=1:2
            c=w35.config('seed',10+i,'duration',250,'tailSteps',5,'windKind',kinds{kk},...
                'windAmpY',1.5,'windBiasY',1);
            [log,~]=w35.run_algorithm(name{1},w35.scenario('static',c),c);
            ex(i)=w35.mop_moe(log,c).energyExcessPercent;
...
for iw=1:size(windKinds,1)
    for name=policies
        ...
        for i=1:2
            c=w35.config('seed',2+i,'duration',800,'tailSteps',60,windKinds{iw,2}{:});
            scn=w35.scenario('static',c);
            [log,info]=w35.run_algorithm(name{1},scn,c);
            m=w35.mop_moe(log,c);
...
% ---- 物理口径核验 ----
c=w35.config('seed',11,'duration',30,'tailSteps',5,'windKind','sin',...
...
cJ=w35.config('seed',3,'duration',800,'plWarmMax',2400,'jumpUpDx',2.7);
scnJ=w35.scenario('jumpUp',cJ);
[logJOn,~]=w35.run_algorithm('purerl_on',scnJ,cJ);
[logJOff,~]=w35.run_algorithm('purerl_off',scnJ,cJ);
ponOK=w35.mop_moe(logJOn,cJ).energyExcessPercent<w35.mop_moe(logJOff,cJ).energyExcessPercent;
...
fprintf(fid,['# 任务3.5检查：预训练purerl拆分(purerl_on在线 / purerl_off离线 / purerl_scratch从零对照) + 继承算法横比\n\n' ...
...
fprintf('检查门槛：%d/%d\n',summary.gatesPassed,summary.gatesTotal);
if summary.gatesPassed<summary.gatesTotal, warning('w35:Checks','Some gates missed.'); end
end
