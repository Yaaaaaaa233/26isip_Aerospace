...
%HYBRID_RUN 任务3.5主角(消融实验)：首飞全速域标定 + task2式在线(冻结曲线, 风修正+信赖域探针)。
% 动机(用户口径, 2026-09-07): "先通过首飞全飞拟合速度-功率曲线, 然后用task2的算法"
% ——把在线算力的重心从"重新学曲线"(task3)移到"跟踪风"(task2)。本策略是该设想的
% 最忠实实现, 任务3.5即"标定后在线该用什么"的受控消融实验(结论见文末与README):
...
while plant.count()<n
    ...
    if kStep==p.swSteps
        [coefs,wSm,fitRms,uLo,uHi,uKept]=w35.fit_curve_wind(calPsi(1:kStep),...
            calV(1:kStep),calP(1:kStep),p);
        uStar=w35.curve_argmin(coefs,uLo,uHi,p,uKept);
        uArg=uStar;
    ...
    if phase==2 && mod(kp,p.hyWindEvery)==0 && nr>=30
        ...
        [dwC,~]=w35.wind_corr(rnPsi(1:nr),rnV(1:nr),rnP(1:nr),coefs,wSm,p);
        wSm=wSm+0.8*dwC;
    ...
    if phase==2 && mod(kp,p.hyRefitEvery)==0 && nr>=30 && wAct
        ...
        vSet=[calV,   rnV(1:nr)];
        pSet=[calP,   rnP(1:nr)];
        [coefsN,~,fitRmsN,uLoN,uHiN,uKeptN]=w35.fit_curve_wind(psSet,vSet,pSet,p,wSm);
        sseOldU=w35.curve_sse(coefs,wSm,psSet,vSet,pSet);
        sseNewU=w35.curve_sse(coefsN,wSm,psSet,vSet,pSet);
        sseOldC=w35.curve_sse(coefs,wSm,calPsi,calV,calP);
        sseNewC=w35.curve_sse(coefsN,wSm,calPsi,calV,calP);
        if sseNewU < sseOldU*p.hySseMargin && sseNewC < sseOldC*1.005
            coefs=coefsN; uLo=uLoN; uHi=uHiN; fitRms=fitRmsN; uKept=uKeptN;
            uStarN=w35.curve_argmin(coefs,uLo,uHi,p,uKept);
        ...
        uArg=w35.curve_argmin(coefs,uLo,uHi,p,uSup);
