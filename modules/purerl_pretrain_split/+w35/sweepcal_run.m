...
%SWEEPCAL_RUN 全速度域快扫标定 + 在线精化(任务3.5主角策略, 纯因果, 曲线未知)。
...
while plant.count()<n
    ...
    if phase==2
        if mod(kStep,p.swRefitEvery)==0 && nr>=30
            ...
            sseOld=w35.curve_sse(coefs,wSm,psSet,vSet,pSet);
            sseNew=w35.curve_sse(coefsN,wSm,psSet,vSet,pSet);
            ...
            uStar=w35.curve_argmin(coefs,uLo,uHi,p,uSup);
            uArg=uStar;
        end
