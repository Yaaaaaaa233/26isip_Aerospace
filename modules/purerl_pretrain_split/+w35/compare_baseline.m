...
if ~any(strcmp(name,{'est','windinfer','sweepcal','rl','purerl'}))  % 2026-09-07精简后口径(各任务按需含purerl/sweepcal/rl)
    error('w35:CompareBaseline','Baseline comparison needs an adaptive algorithm, got %s.',name);
end
cB=c; cB.seed=c.seed;
[logA,~]=w35.run_algorithm(name,scn,c);
mA=w35.mop_moe(logA,c);
[logB,~]=w35.run_algorithm('openloop',scn,cB);
mB=w35.mop_moe(logB,cB);
