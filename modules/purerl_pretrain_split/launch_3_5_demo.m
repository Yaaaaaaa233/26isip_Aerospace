function fig = launch_3_5_demo(visible)
%LAUNCH_3_3_DEMO 任务3.5标定+风推断混合面板：控制台(主) + 飞机模型 + 环境模型。
...
% —— 自愈暂存(2026-09-08): 绕开MATLAB目录缓存滞后导致的 w35.* 解析失败 ——
if exist('w35.fit_curve_wind','file')~=2
    stg=fullfile(tempdir,['w35stage_' datestr(now,'yyyymmddHHMMSS') '_' num2str(randi(8999)+1000)]);
    copyfile(fullfile(root,'+w35'),fullfile(stg,'+w35'));
    addpath(stg); clear functions; rehash;
end
fig=uifigure('Name','任务3.5风速推断寻优：空速=地速−风速(顺风右移/逆风左移) × 风不影响运动 × 七种风场(动态演示)',...
...
header=uilabel(outer,'Text','任务3.5程序 | 空速=地速−风速(地速6+顺风3→空速3) × 曲线=空速曲线(固定) × 地速曲线顺风右移/逆风左移 × 曲线未知(调速→功率黑盒) × 首飞3→12全速域快扫标定 × MOE=纯能耗(Emin/Eactual)',...
...
algorithm=choice(g,'控制策略',1,{'sweepcal全速域标定+精化','hybrid标定+风推断混合',...
    ...
    {'sweepcal','hybrid','rl','purerl_on','purerl_off','purerl_scratch','openloop','windinfer','est','known'},'purerl_off');
...
logMsg('任务3.5预训练purerl拆分程序就绪 | 主角: purerl_on预训练在线 / purerl_off预训练离线(试飞时段预训练收敛后部署, 不占评测预算) | 对照: purerl_scratch从零在线 + sweepcal/rl/hybrid/openloop + oracle参照');
...
prepare();
    ...
    function updateModules(k)
        ...
        [Wx,Wy,Vx,Vy]=w35.wind_field(scn,tNow,psi);
        ...
        if strcmp(c.windKind,'sector')   % 扇区风: 横轴=航向, 当前点=此刻航向处的风
            ...
            [WxC,WyC]=w35.wind_field(scn,0,deg2rad(psd));
            ...
            [WxC,WyC]=w35.wind_field(scn,tt,0);
    ...
    function buildConfig()
        c=w35.config('initialSpeed',initial.Value,'noiseSigma',noise.Value,'curveCase',curveC.Value,...
    ...
    function windPreview()
        ...
        if strcmp(c.windKind,'sector')
            ...
            [WxP,WyP]=w35.wind_field(pvScn,tt,0);
    ...
    function drawCasePreview()
        ...
        for q=1:3
            cc=w35.config(c,'curveCase',cv(q));
            hds(q).XData=vv; hds(q).YData=w35.base_curve(vv,cc)*c.pHover;
    ...
    function redraw()
        ...
        [WxK,WyK,VxK,VyK]=w35.wind_field(scn,tNow,psiK);
        vv=linspace(c.lower,c.upper,400);
        if vis
            uu=hypot(vv*cos(psiK)-VxK,vv*sin(psiK)-VyK);
            h.curve.XData=vv; h.curve.YData=(w35.base_curve(uu-L.shiftDx(k),c)+L.shiftDy(k))*c.pHover;
            h.curveAir.XData=vv; h.curveAir.YData=(w35.base_curve(vv-L.shiftDx(k),c)+L.shiftDy(k))*c.pHover;
