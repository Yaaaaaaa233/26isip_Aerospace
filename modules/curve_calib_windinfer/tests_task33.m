function tests = tests_task33()
%TESTS_TASK31 任务3.3单元测试: 曲线未知(黑盒调速→功率) + 全速度域快扫标定
...
t=[0 1.37 9.1 22.8];
% const: 恒等于偏置
c=w33.config(wind{:},'windKind','const'); scn=w33.scenario('static',c);
[wx,wy]=w33.wind_field(scn,[0 12.3 40],0);
tc.verifyEqual(wx,[3 3 3],'AbsTol',1e-12);
tc.verifyEqual(wy,[1 1 1],'AbsTol',1e-12);
% sin: 与闭式一致(任务4/5/8原口径)
c=w33.config(wind{:},'windKind','sin'); scn=w33.scenario('static',c);
[wx,wy]=w33.wind_field(scn,t,0);
...
c=w33.config(wind{:},'windKind','square','squareEdge',4); scn=w33.scenario('static',c);
tp=pi/2/0.4;
[wx,~]=w33.wind_field(scn,tp,0);
tc.verifyEqual(wx,5,'AbsTol',1e-9);
tt=0:0.01:40;
[wx,wy]=w33.wind_field(scn,tt,0);
...
c=w33.config(wind{:},'windKind','triangle'); scn=w33.scenario('static',c);
[wx,~]=w33.wind_field(scn,tp,0);
tc.verifyEqual(wx,5,'AbsTol',1e-9);
t2=[0 tp/2 tp];
[wx,~]=w33.wind_field(scn,t2,0);
slope=(wx(2)-wx(1))/(t2(2)-t2(1));
tc.verifyEqual(slope,2*0.4*2/pi,'AbsTol',1e-9);
% turb: 确定性(同种子同序列)+平稳统计(std≈windAmp, 均值≈0)
wA=2.0;
c1=w33.config(wind{:},'windKind','turb','windAmp',wA,'turbTheta',0.2,'duration',4000);
s1=w33.scenario('static',c1); s2=w33.scenario('static',c1);
...
c=w33.config(wind{:},'windKind','composite','turbStd',0); scn=w33.scenario('static',c);
[wx,wy]=w33.wind_field(scn,t,0);
tc.verifyEqual(wx,2*sin(0.4*t)+3,'AbsTol',1e-12);
c=w33.config(wind{:},'windKind','composite','turbStd',0.3,'seed',5,'duration',400);
scn=w33.scenario('static',c);
tt2=0:0.01:400;
[wx,~]=w33.wind_field(scn,tt2,0);
resid=wx-(2*sin(0.4*tt2)+3);
tc.verifyLessThan(abs(std(resid(10000:end))-0.3),0.05,'复合风湍流分量std应≈turbStd');
% sector: 与时间无关、周期2π、绕偏置幅值=A
c=w33.config(wind{:},'windKind','sector'); scn=w33.scenario('static',c);
[wx1,wy1]=w33.wind_field(scn,0,0);
[wx2,wy2]=w33.wind_field(scn,137.9,0);
tc.verifyEqual(wx1,wx2); tc.verifyEqual(wy1,wy2);
[wx3,wy3]=w33.wind_field(scn,0,2*pi);
tc.verifyEqual(wx3,wx1,'AbsTol',1e-12); tc.verifyEqual(wy3,wy1,'AbsTol',1e-12);
[wxp,~]=w33.wind_field(scn,0,pi);   % ψ=π: Wx=B−A·cos(π)=B+A
tc.verifyEqual(wxp,5,'AbsTol',1e-12);
[wxh,wyh]=w33.wind_field(scn,0,pi/2); % ψ=π/2: Wx=B−A·cos(π/2)=B, Wy=D+C·sin(π/2)=D+C
...
c=w33.config(wind{:},'windKind','sector');
scn=w33.scenario('static',c);
psd=deg2rad(0:1:360);
[wx,wy]=w33.wind_field(scn,0,psd);
...
c=w33.config(wind{:}); scn=w33.scenario('static',c);
t=3.7;
[wx,wy,vx,vy]=w33.wind_field(scn,t,0);
...
c=w33.config('seed',11,'duration',30,'tailSteps',5,wind{:});
[log,~]=w33.run_algorithm('openloop',w33.scenario('static',c),c);
...
c=w33.config('seed',11,'duration',5,'tailSteps',1,'turnRadius',600,...
...
cH=w33.config('seed',11,'duration',5,'tailSteps',1,'turnRadius',600,...
    'initialSpeed',6,'openLoopV',6,windH{:});
[logH,~]=w33.run_algorithm('openloop',w33.scenario('static',cH),cH);
...
c=w33.config('seed',11,'duration',20,'tailSteps',5,'initialSpeed',6.3,'openLoopV',6.3,wind{:});
[log,~]=w33.run_algorithm('openloop',w33.scenario('static',c),c);
P63=w33.base_curve(6.3,c);   % 零风+恒地速: 空速=地速, 功率=base_curve(6.3)
tc.verifyEqual(log.powerTrue,P63*ones(height(log),1),'AbsTol',1e-9);
end

function test_ground_curve_optimum_closed_form(tc)
wind={'windAmp',2,'windOmega',0.08,'windBias',3,'windAmpY',1.5,'windOmegaY',0.13,'windBiasY',1};
c=w33.config('seed',11,'duration',40,'tailSteps',10,wind{:});
scn=w33.scenario('static',c);
...
for k=1:n
    ...
    [~,~,VxE,VyE]=w33.wind_field(scn,tEnd,psiE);
...
c=w33.config('seed',11,'duration',60,'tailSteps',5,'initialSpeed',6.3,'openLoopV',6.3,wind{:});
[log,~]=w33.run_algorithm('openloop',w33.scenario('static',c),c);
...
c0=w33.config('seed',11,'duration',60,'tailSteps',5,'initialSpeed',6.3,'openLoopV',6.3,wind0{:});
[log0,~]=w33.run_algorithm('openloop',w33.scenario('static',c0),c0);
...
for r=[0.95 0.90 0.85]
    c=w33.config('curveCase',r);
    tc.verifyEqual(w33.base_curve(0,c),1.0,'AbsTol',1e-9);
    tc.verifyEqual(w33.base_curve(c.optimum0,c),r,'AbsTol',1e-9);
    vv=0:0.005:20; Pw=w33.base_curve(vv,c);
    [~,im]=min(Pw);
...
tc.verifyEqual(w33.base_curve(20,c)*c.pHover,134.5,'AbsTol',3.5);
end
...
for x=[2.0 6.3 10.5 18.0]
    h=1e-6;
    fd=(w33.base_curve(x+h,c)-w33.base_curve(x-h,c))/(2*h);
    tc.verifyEqual(w33.base_curve_grad(x,c),fd,'AbsTol',1e-7);
end
end

function test_moe_identity_and_case_in_plant(tc)
c=w33.config('seed',11,'curveCase',0.85);
[log,~]=w33.run_algorithm('openloop',w33.scenario('static',c),c);
m=w33.mop_moe(log,c);
...
c=w33.config('initialSpeed',10,'openLoopV',4,'windAmp',0,'windBias',0,...
    'windAmpY',0,'windBiasY',0,'windKind','const','latencySec',0.3);
[log,~]=w33.run_algorithm('openloop',w33.scenario('static',c),c);
tc.verifyEqual(log.speed(1),8.6,'AbsTol',1e-9);
% 任务3.3放宽: τ=0(无时延对照) — 指令立即释放, 仅受限幅: 第1步末 10-2×1=8
c0=w33.config('initialSpeed',10,'openLoopV',4,'windAmp',0,'windBias',0,...
...
policies={'openloop','est','windinfer','hybrid','sweepcal','rl','known'};
for name=policies
    c=w33.config('seed',11,'duration',400);
...
policies={'openloop','known','sweepcal','rl','hybrid'};
for kk=1:numel(kinds)
    for pp=1:numel(policies)
        c=w33.config('seed',11,'duration',250,'tailSteps',5,'windKind',kinds{kk},...
            'windAmpY',1.5,'windBiasY',1);
        [log,~]=w33.run_algorithm(policies{pp},w33.scenario('static',c),c);
...
c=w33.config('initialSpeed',6.3,'openLoopV',6.3,'windAmp',0,'windBias',0,...
...
c=w33.config('initialSpeed',6.3,'openLoopV',6.3,'windAmp',0,'windBias',0,...
    'windAmpY',0,'windBiasY',0,'windKind','const');
[log,~]=w33.run_algorithm('openloop',w33.scenario('static',c),c);
tc.verifyEqual(w33.mop_moe(log,c).MOE_energy,1.0,'AbsTol',1e-9);
end

function test_known_oracle_information_value(tc)
exK=zeros(1,5); exO=zeros(1,5);
for i=1:5
    c=w33.config('seed',10+i);
    scn=w33.scenario('static',c);
    [log,~]=w33.run_algorithm('known',scn,c);
    exK(i)=w33.mop_moe(log,c).energyExcessPercent;
    [log,~]=w33.run_algorithm('openloop',scn,c);
    exO(i)=w33.mop_moe(log,c).energyExcessPercent;
end
tc.verifyTrue(mean(exK)<1.0);
tc.verifyTrue(mean(exO)-mean(exK)>3.0);
end

%% ---------- 任务3.3核心: u*固定 + 风推断寻优(windinfer) ----------
function test_ustar_fixed_pmin_constant(tc)
% 任务3.3核心设定: 无平移调度 → 空速最优点u*固定, Pmin恒定不随时间变化;
% 地速最优v*仍随风变化(这正是需要寻优的原因)。
wind={'windKind','sin','windAmp',2,'windBias',3,'windAmpY',1.5,'windOmegaY',0.13,'windBiasY',1};
c=w33.config('seed',7,'duration',300,'tailSteps',60,wind{:});
[log,~]=w33.run_algorithm('openloop',w33.scenario('static',c),c);
...
c=w33.config('seed',3,'windKind','const','windBias',3.5,'windDirDeg',40);
scn=w33.scenario('static',c);
[log,info]=w33.run_algorithm('windinfer',scn,c);
m=w33.mop_moe(log,c);
...
c=w33.config('seed',3,'windKind','const','windBias',0,'windDirDeg',0);
scn=w33.scenario('static',c);
[log,info]=w33.run_algorithm('windinfer',scn,c);
m=w33.mop_moe(log,c);
...
c=w33.config('seed',3,wcfg{:});
scn=w33.scenario('static',c);
[logI,~]=w33.run_algorithm('windinfer',scn,c);
[logO,~]=w33.run_algorithm('openloop',scn,c);
mI=w33.mop_moe(logI,c); mO=w33.mop_moe(logO,c);
...
c=w33.config('seed',3,'windKind','const','windBias',5.5,'windDirDeg',200);
scn=w33.scenario('static',c);
[log,info]=w33.run_algorithm('windinfer',scn,c);
m=w33.mop_moe(log,c);
...
tc.verifyLessThan(abs(mean(log.powerTrue)-w33.base_curve(3.5,c)),1e-3,'悬停功率应=J0(|w|)');
c0=w33.config('seed',3,'windKind','const','windBias',0,'openLoopV',6.3);
[log0,~]=w33.run_algorithm('openloop',w33.scenario('static',c0),c0);
tc.verifyEqual(w33.mop_moe(log0,c0).MOE_energy,1.0,'AbsTol',1e-9,'零风匀速@u*应=最优');
end
...
c=w33.config('seed',3);
scn=w33.scenario('static',c);
[log,~]=w33.run_algorithm('windinfer',scn,c);
m=w33.mop_moe(log,c);
...
% 任务3.3主角: 恒定风下首飞标定+task2式在线风修正——应辨识出 û*≈6.3 与风矢量,
% 整体不劣于sweepcal的重拟合链(容差), 显著优于开环。
c=w33.config('seed',3,'windKind','const','windBias',3.5,'windDirDeg',40,'duration',800);
scn=w33.scenario('static',c);
[log,info]=w33.run_algorithm('hybrid',scn,c);
m=w33.mop_moe(log,c);
[logS,~]=w33.run_algorithm('sweepcal',scn,c);
mS=w33.mop_moe(logS,c);
[logO,~]=w33.run_algorithm('openloop',scn,c);
mO=w33.mop_moe(logO,c);
...
c=w33.config('seed',3,'windKind','const','windBias',0,'windDirDeg',0,'duration',600);
scn=w33.scenario('static',c);
[log,info]=w33.run_algorithm('hybrid',scn,c);
m=w33.mop_moe(log,c);
...
% 变风: 冻结曲线在线的负结果复现——hybrid应显著劣于sweepcal(标定被漂移风污染,
% 冻结曲线后û*无再锚定)。这是任务3.3的核心发现之一, 须稳定复现。
wcfg={'windKind','composite','windBias',2.5,'windAmp',1.5,'windOmega',0.08,...
    'windBiasY',0,'windAmpY',0,'turbStd',0.3};
c=w33.config('seed',3,wcfg{:},'duration',800);
scn=w33.scenario('static',c);
[logH,~]=w33.run_algorithm('hybrid',scn,c);
[logS,~]=w33.run_algorithm('sweepcal',scn,c);
mH=w33.mop_moe(logH,c); mS=w33.mop_moe(logS,c);
tc.verifyGreaterThan(mH.energyExcessPercent,mS.energyExcessPercent,...
    '变风下冻结曲线在线应劣于sweepcal(负结果复现)');
end
...
c=w33.config('seed',3,'windKind','const','windBias',3.5,'windDirDeg',40,'duration',600);
scn=w33.scenario('static',c);
[log,~]=w33.run_algorithm('hybrid',scn,c);
tags=string(log.tag);
...
c=w33.config('seed',3,'windKind','const','windBias',3.5,'windDirDeg',40,'duration',800);
scn=w33.scenario('static',c);
[log,info]=w33.run_algorithm('sweepcal',scn,c);
m=w33.mop_moe(log,c);
[logO,~]=w33.run_algorithm('openloop',scn,c);
mO=w33.mop_moe(logO,c);
tc.verifyLessThan(abs(info.uStar-c.optimum0),0.6,'û*辨识误差应<0.6 m/s');
...
c=w33.config('seed',3,wcfg{:},'duration',800);
scn=w33.scenario('static',c);
[logI,~]=w33.run_algorithm('sweepcal',scn,c);
[logO,~]=w33.run_algorithm('openloop',scn,c);
mI=w33.mop_moe(logI,c); mO=w33.mop_moe(logO,c);
...
c=w33.config('seed',3,'duration',600);
scn=w33.scenario('static',c);
[log,info]=w33.run_algorithm('rl',scn,c);
m=w33.mop_moe(log,c);
...
for kk=1:numel(kinds)
    c=w33.config('seed',11,'duration',250,'tailSteps',5,'windKind',kinds{kk},...
        'windAmpY',1.5,'windBiasY',1);
    [log1,~]=w33.run_algorithm('sweepcal',w33.scenario('static',c),c);
    [log2,~]=w33.run_algorithm('rl',w33.scenario('static',c),c);
