function fig = launch_3_1_demo(visible)
%LAUNCH_2_1_DEMO 任务3.1风速推断寻优面板：控制台(主) + 飞机模型 + 环境模型。
...
% —— 自愈暂存(2026-09-08): 部分MATLAB实例对历史会话新建的包文件存在目录缓存滞后,
% 启动后报"无法解析名称 w31.fit_curve_wind"。这里检测解析失败就把+w31完整复制到
% 全新临时目录并前置到路径, 强制重新解析(新目录无陈旧缓存)。任何入口均自愈。
if exist('w31.fit_curve_wind','file')~=2
    stg=fullfile(tempdir,['w31stage_' datestr(now,'yyyymmddHHMMSS') '_' num2str(randi(8999)+1000)]);
    copyfile(fullfile(root,'+w31'),fullfile(stg,'+w31'));
    addpath(stg); clear functions; rehash;
end
fig=uifigure('Name','任务3.1风速推断寻优：空速=地速−风速(顺风右移/逆风左移) × 风不影响运动 × 七种风场(动态演示)',...
...
header=uilabel(outer,'Text','任务3.1程序 | 空速=地速−风速(地速6+顺风3→空速3) × 曲线=空速曲线(固定) × 地速曲线顺风右移/逆风左移 × 曲线未知(调速→功率黑盒) × 首飞3→12全速域快扫标定 × MOE=纯能耗(Emin/Eactual)',...
...
algorithm=choice(g,'控制策略',1,{'sweepcal全速域标定+精化(3.1主角)','rl在线策略梯度(对照)',...
    'openloop开环固定基线',...
    'windinfer已知曲线oracle参照','est已知曲线EKF(参照)','known已知风+曲线oracle上限'},...
    {'sweepcal','rl','openloop','windinfer','est','known'},'sweepcal');
...
logMsg('任务3.1风场模型库程序就绪 | 七种风场: 恒定/双正弦/方波(软边)/三角/湍流(OU)/复合(推荐)/扇区(随航向) | 下拉选中即预览');
...
if exist(fullfile(root,'results','report.md'),'file')
    ...
    logMsg('尚未生成验收报告: 命令行运行 run_task31_checks 后点"载入验收报告"');
end
prepare();
    ...
    function updateModules(k)
        ...
        [Wx,Wy,Vx,Vy]=w31.wind_field(scn,tNow,psi);
        ...
        if strcmp(c.windKind,'sector')   % 扇区风: 横轴=航向, 当前点=此刻航向处的风
            ...
            [WxC,WyC]=w31.wind_field(scn,0,deg2rad(psd));
            ...
            [WxC,WyC]=w31.wind_field(scn,tt,0);
    ...
    function buildConfig()
        c=w31.config('initialSpeed',initial.Value,'noiseSigma',noise.Value,'curveCase',curveC.Value,...
    ...
    function windPreview()
        ...
        if strcmp(c.windKind,'sector')
            ...
            [WxP,WyP]=w31.wind_field(pvScn,tt,0);
    ...
    function drawCasePreview()
        ...
        for q=1:3
            cc=w31.config(c,'curveCase',cv(q));
            hds(q).XData=vv; hds(q).YData=w31.base_curve(vv,cc)*c.pHover;
    ...
    function redraw()
        ...
        [WxK,WyK,VxK,VyK]=w31.wind_field(scn,tNow,psiK);
        vv=linspace(c.lower,c.upper,400);
        if vis
            uu=hypot(vv*cos(psiK)-VxK,vv*sin(psiK)-VyK);
            h.curve.XData=vv; h.curve.YData=(w31.base_curve(uu-L.shiftDx(k),c)+L.shiftDy(k))*c.pHover;
            h.curveAir.XData=vv; h.curveAir.YData=(w31.base_curve(vv-L.shiftDx(k),c)+L.shiftDy(k))*c.pHover;
        ...
        if any(strcmp(algorithm.Value,{'sweepcal','rl'})) && isstruct(info) && isfield(info,'coefs') ...
    ...
    function loadReport(varargin)
        ...
        if ~exist(rp,'file')
            logMsg('未找到验收报告: 请先在命令行运行 run_task31_checks');
            return;
        end
        lines=readlines(rp);
