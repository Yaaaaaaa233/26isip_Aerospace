...
%SWEEPCAL_RUN 全速度域快扫标定 + 在线精化(任务3.1主角策略, 纯因果, 曲线未知)。
...
while plant.count()<n
    ...
    if kStep==floor(p.swSteps/2) || kStep==p.swSteps
        [coefs,wSm,fitRms,uLo,uHi]=w31.fit_curve_wind(calPsi(1:kStep),...
            calV(1:kStep),calP(1:kStep),p);
        uStar=w31.curve_argmin(coefs,uLo,uHi,p);
        if kStep==p.swSteps
    ...
    if phase==2 && mod(kStep,p.swRefitEvery)==0 && nr>=30
        ...
        [dwC,~]=w31.wind_corr(rnPsi(1:nr),rnV(1:nr),rnP(1:nr),coefs,wSm,p);
        wSm=wSm+0.8*dwC;
    end
    if phase==2
        if mod(kStep,p.swRefitEvery)==0 && nr>=30
            ...
            [coefsN,~,fitRmsN,uLoN,uHiN]=w31.fit_curve_wind(psSet,vSet,pSet,p,wSm);
            sseOld=w31.curve_sse(coefs,wSm,psSet,vSet,pSet);
            sseNew=w31.curve_sse(coefsN,wSm,psSet,vSet,pSet);
