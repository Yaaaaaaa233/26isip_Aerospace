%TABLE_3X3 任务3.1 三乘三验证表：飞机模式 × 风况, 单元格=MOE(纯能耗 Emin/Eactual)。
% 飞机模式: 悬停(v=0) / 匀速转圈(地速≡名义6.3) / 变速转圈(sweepcal, 曲线未知)。
% 风况: 无风 / 恒定风(3.5 m/s @40°) / 变风(composite)。
% 任务3.1特有口径: 控制器不知道曲线(u*未知)也不知道风——匀速行飞名义6.3是
% "仅依赖单一速度点"的用户基线; sweepcal含首飞标定学费(一次性)。
...
for iw=1:3
    for im=1:3
        vals=[];
        for sd=[3 6 9]
            c=w31.config('seed',sd,'duration',800,'tailSteps',60,windCfg{iw}{:});
            scn=w31.scenario('static',c);
            if im==3
                [log,~]=w31.run_algorithm('sweepcal',scn,c);
            else
                if im==1, olv=0; else, olv=6.3; end
                c2=w31.config(c,'openLoopV',olv);
                [log,~]=w31.run_algorithm('openloop',scn,c2);
            end
    ...
    for sd=[3 6 9]
        c=w31.config('seed',sd,'duration',800,'tailSteps',60,windCfg{iw}{:});
        [log,~]=w31.run_algorithm('known',w31.scenario('static',c),c);
...
fprintf(fid,['\n判读: 匀速转圈@名义6.3 即用户所说"仅依赖单一速度点"的基线——无风时恰好最优, ',...
    ...
    '且无需任何曲线先验。RL对照(策略梯度)为诚实负结果, 详见 run_task31_checks 报告。\n']);
