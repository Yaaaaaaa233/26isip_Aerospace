...
%QA_UNIFIED_DEMO 任务3面板自检：算法/场景切换、参数变更、开关不改变
% 日志、播放暂停重置、MOP/MOE读出与导出。
root=fileparts(mfilename('fullpath')); addpath(root);
folder=fullfile(root,'results','ui_qa'); if ~exist(folder,'dir'), mkdir(folder); end
fig=launch_task3_demo('on'); cleanup=onCleanup(@()close(fig)); %#ok<NASGU>
u=fig.UserData;
...
u.controls.windSpeed.Value=0; u.controls.windSpeed.ValueChangedFcn([],[]);
...
u.controls.windSpeed.Value=5.0; u.controls.windSpeed.ValueChangedFcn([],[]);
u.controls.circleT.Value=60; u.controls.circleT.ValueChangedFcn([],[]);
u.prepare(); u.finish();
Lw=u.getLog();
assert(height(Lw)==400,'风场参数生效后日志仍为400步');
vExp=6+u.controls.shiftDx.Value+wsearch.wind_truth(wsearch.scenario('static',wsearch.config('windSpeed',5,'circlePeriod',60)),Lw.time(400));
assert(abs(Lw.optimumTrue(400)-vExp)<1e-9,'风场真值最优应随航向计算');
u.controls.windSpeed.Value=3.0; u.controls.windSpeed.ValueChangedFcn([],[]);
