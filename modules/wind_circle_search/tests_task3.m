function tests = tests_task3
%TESTS_TASK3 统一程序单元测试：对象/平移/黑箱边界/MOE定义/算法行为/开关。
...
c=wsearch.config('shiftTime',100);
...
c=wsearch.config('duration',200,'tailSteps',60,'seed',11,'windSpeed',0);
...
c=wsearch.config('duration',200,'tailSteps',60,'seed',3,'windSpeed',0);
scn=wsearch.scenario('static',c);
...
for seed=11:16
    c=wsearch.config('seed',seed,'windSpeed',0);
    scn=wsearch.scenario('static',c);
...
c=wsearch.config('seed',11,'windSpeed',0);
scn=wsearch.scenario('static',c);
...
% 环境模型: 恒定风×圆周 → 周期偏移; 风速=0退化为任务1+2口径
c=wsearch.config('windSpeed',3,'circlePeriod',80,'windDirDeg',0);
scn=wsearch.scenario('static',c);
[dx0,dy0,psi0]=wsearch.wind_truth(scn,0);
verifyEqual(t,psi0,0);                       % t=0 航向迎风(逆风最大)
verifyEqual(t,dx0,c.windDxGain*3,'AbsTol',1e-12);
verifyEqual(t,dy0,c.windDyGain*3,'AbsTol',1e-12);
[dxh,dyh,psih]=wsearch.wind_truth(scn,40);   % 半圈: 顺风
verifyEqual(t,psih,pi,'AbsTol',1e-9);
verifyEqual(t,dxh,-dx0,'AbsTol',1e-12); verifyEqual(t,dyh,-dy0,'AbsTol',1e-12);
[dxq,dyq,psiq]=wsearch.wind_truth(scn,20);   % 1/4圈: 侧风, 零投影
verifyEqual(t,psiq,pi/2,'AbsTol',1e-9);
verifyEqual(t,dxq,0,'AbsTol',1e-12); verifyEqual(t,dyq,0,'AbsTol',1e-12);
c0=wsearch.config('windSpeed',0);
scn0=wsearch.scenario('static',c0);
[dxN,dyN,psiN]=wsearch.wind_truth(scn0,[0 40]);  % 风速=0兼容
verifyEqual(t,dxN,[0 0]); verifyEqual(t,dyN,[0 0]); verifyEqual(t,psiN,[0 0]);
end
...
c=wsearch.config('duration',400,'tailSteps',60,'seed',3,'windSpeed',3);
scn=wsearch.scenario('static',c);
[log,~]=wsearch.run_algorithm('fixed',scn,c);
m=wsearch.mop_moe(log,c);
...
c=wsearch.config('seed',11,'windSpeed',3,'circlePeriod',80);
scn=wsearch.scenario('static',c);
[log,~]=wsearch.run_algorithm('ea_multistart',scn,c);
tail=341:400;
err=abs(log.estimate(tail)-log.optimumTrue(tail));
