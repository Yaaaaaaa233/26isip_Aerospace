function fig = launch_task3_demo(visible)
%LAUNCH_TASK3_DEMO 任务3动画面板：环境(恒定风×圆周) × 崎岖对象 × MOP/MOE。
...
fig=uifigure('Name','任务3风场圆周寻优：环境风×盘旋×MOP/MOE(动态演示)',...
...
header=uilabel(outer,'Text','任务3风场圆周寻优程序 | 环境风×盘旋×崎岖对象·MOP/MOE·代理模型',...
...
g=uigridlayout(form,[17 2]); g.ColumnWidth={140,'1x'};
g.RowHeight=[repmat({30},1,13),{26,30,26,30}];
...
windSpeed=number(g,'环境风速 / m/s',12,3.0,[0 10]);
circleT=number(g,'盘旋周期 / s',13,80,[10 400]);
truth=uicheckbox(g,'Text','显示评价器曲线与真值','Value',true); put(truth,14,[1 2]);
energy=uicheckbox(g,'Text','搜索能耗计入评价(续航口径)','Value',true); put(energy,15,[1 2]);
speedLabel=uilabel(g,'Text','播放速度: 1.00x','FontName','Microsoft YaHei','FontSize',11); put(speedLabel,16,1);
speedSlider=uislider(g,'Limits',[0.5 8],'Value',1,'MajorTicks',[.5 1 2 4 8],...
    'MajorTickLabels',{'0.5x','1x','2x','4x','8x'}); put(speedSlider,16,2);
...
controls=struct('algorithm',algorithm,'scenario',scenarioC,'initial',initial,...
    ...
    'windSpeed',windSpeed,'circleT',circleT,'truth',truth,'energy',energy,...
...
for cn={algorithm,scenarioC,initial,noise,ripA1,ripL1,ripA2,ripL2,shiftTime,shiftDx,seed,windSpeed,circleT}
...
logMsg('任务3程序就绪 | 算法: tracker平移跟踪 / esc连续ESC | 环境: 恒定风×盘旋');
logMsg('提示: 风速=0即任务1+2口径; 场景/平移/能耗开关/日志栏功能与统一程序一致');
if exist(fullfile(root,'results','report.md'),'file')
    loadReport();
else
    logMsg('尚未生成验收报告: 命令行运行 run_task3_acceptance 后点"载入验收报告"');
end
prepare();
    ...
    function buildConfig()
        cc=wsearch.config('initialSpeed',initial.Value,'noiseSigma',noise.Value,...
            ...
            'windSpeed',windSpeed.Value,'circlePeriod',circleT.Value);
        % 跳变幅值: 场景映射
