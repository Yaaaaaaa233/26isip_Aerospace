...
s0=task1.brent_search(f,p.lower,p.upper,p.tol,min(p.maxSearchEval,n-1));
appendBrackets(s0);
lockV=s0.x; info.lockV=lockV;
hold=0;
while plant.count()<n
    if hold<p.probePeriod
    ...
    hold=0;
    pm=plant.q(lockV-p.probeDelta,'probe'); plant.amendEstimate(lockV);
    pp=plant.q(lockV+p.probeDelta,'probe'); plant.amendEstimate(lockV);
    info.probePairs=info.probePairs+1;
    g=(pp-pm)/(2*p.probeDelta);
    trigger=false;
    if ~drift && abs(g)>=p.slopeThresh
        drift=true; trigger=true;          % 进入漂移模式
    elseif drift
        if abs(g)>=p.slopeRelease
    ...
    span=p.bracketSpan0; sr=[];
    for attempt=1:p.bracketRetry+1
        if plant.count()>=n, break; end
        a=max(p.lower,lockV-span); b=min(p.upper,lockV+span);
        f=task1.search_query(plant);
        sr=task1.brent_search(f,a,b,p.tol,min(p.maxSearchEval,n-plant.count()));
        appendBrackets(sr);
        edge=0.05*(b-a);
        if abs(sr.x-a)>edge && abs(sr.x-b)>edge, break; end
        span=span*p.bracketGrow;
    end
