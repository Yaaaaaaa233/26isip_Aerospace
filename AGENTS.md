...
- `models/px4_x8/` — PX4 X8 Simulink 验证平台，阶段 0、M0-A、M0-B、M0-C、M1、M2 核心成果已完成并放行；第五轮独立复验的标准分段矩阵 15/15 PASS，但发现非有限调用者状态恢复和 stage 证据来源绑定缺陷，验收自动化为 PARTIAL（`docs/evidence/M2_REACCEPT_ROUND5_CODEX_20260902.md`）。当前下一项为 M3 速度与转速比交替协同优化；上述缺陷须在下一里程碑放行前关闭。注意：本机 R2022b 单进程长序列仿真有堆损坏风险，批量验收用 `verify_m2_round4_closure` 的分段模式（stage 逐段执行）。
