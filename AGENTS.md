...
- `models/px4_x8/` — PX4 X8 Simulink 验证平台，阶段 0、M0-A、M0-B、M0-C、M1 已完成；M2 已接入受约束 eta 分配器与原生转速比 ESC（快照 `air_m2.slx`），修订数值协议通过，但第二轮复验重新打开链级 global 隔离与九场景硬断言问题；关闭后再做 M3 `.slx` 结构集成。
