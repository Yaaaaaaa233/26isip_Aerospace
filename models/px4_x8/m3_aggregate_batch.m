...
assert(strcmp(info.gitCommit, live.gitCommit), 'air:M3Agg:SourceMismatch', ...
    'manifest commit %s differs from live HEAD %s -- mixed batch', ...
    info.gitCommit, live.gitCommit);
...
for d = 1:numel(segDirs)
    ...
    assert(isfield(r, 'binding') && isfield(r.binding, 'gitCommit') && ...
        ...
        isfield(r, 'attempts'), 'air:M3Agg:NoBinding', ...
    ...
    assert(strcmp(r.binding.gitCommit, live.gitCommit) && ...
        strcmp(r.bindingExit.gitCommit, live.gitCommit), ...
        'air:M3Agg:SourceMismatch', ...
        'segment %s commit (%s / exit %s) differs from live HEAD %s', ...
...
segTable = struct();
for d = 1:numel(segs)
    segTable(d) = struct('segName', segs(d).segName, ...
        'dir', string(segs(d).dir), 'runId', segs(d).binding.runId, ...
        'attempts', segs(d).attempts, ...
        'commit', segs(d).binding.gitCommit);
end
