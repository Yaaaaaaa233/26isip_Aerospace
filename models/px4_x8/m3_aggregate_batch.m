...
for j = 1:numel(c.arms)
    % the VERDICT comes from the segment result's runs table (the stage
    % verdict layer, rules section 2 rule 6: FAIL rows in a stage result
    % must not aggregate); gate DATA later comes from the per-arm archive
