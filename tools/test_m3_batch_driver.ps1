...
<#
.SYNOPSIS
    Driver-level tests for the M3 bounded-retry batch driver (round-3
    M3-R2-F4; rules v1.7 section 2 rule 7h).

.DESCRIPTION
    Dot-sources tools/run_m3_batch.ps1 with -SourceOnly and exercises
    ...
    S7  aggregate-style archive evidence   -> a NEW directory under $DoneDir counts as the
                                             fresh done marker; no new dir -> retry
    S8  log encoding round trip (rule 9)   -> a native command emitting ANSI-page CJK
    ...
    Run under BOTH PowerShell 7 and Windows PowerShell 5.1:
        pwsh -File tools\test_m3_batch_driver.ps1
        powershell -File tools\test_m3_batch_driver.ps1
#>

$driver = Join-Path $PSScriptRoot 'run_m3_batch.ps1'
. $driver -SourceOnly
...
$base = Join-Path ([System.IO.Path]::GetTempPath()) ('m3_driver_test_' + (Get-Date -Format 'yyyyMMdd_HHmmss'))
...
$invoke = { param($logPath)
    ...
    Start-Sleep -Milliseconds 20   # tick spacing: marker must be strictly newer than t0
...
Assert-True ($n -eq 1) 'S2 post-stamp crash accepted on attempt 1 (rules v1.7 rule 7e)'
...
# --- S7: aggregate-style evidence -- a NEW directory under $DoneDir ---------
...
# page bytes; the round-9 M2 logs captured under a UTF-8 console were
# full of U+FFFD).
$cn = -join @([char]0x4E2D, [char]0x6587, [char]0x7B2C, [char]0x4E09, [char]0x9636, [char]0x6BB5, [char]0x6D4B, [char]0x8BD5)
