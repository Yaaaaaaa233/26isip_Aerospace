...
状态：**核心实现与修订数值协议已放行；第八轮发现（R8-F1/F2/F3）已修复并经 48/48 针对性矩阵关闭验证（2026-09-02，runId `72300de6` @ `6f6672c`）**。stage 源码指纹、verdict 逐行硬断言与 attempt 证据硬断言（存在/正整数/`manifest.maxAttempts` 上限/与持久标记一致，六类负向证明）均有效；有界重试驱动入库 `tools/run_m2_batch.ps1`（8 场景驱动层测试 + 正式批次内受控击杀→自动重试→c5 attempt=2 实绩；新鲜完成证据权威、退出码仅参考——matlab 启动器对堆崩溃子进程报 rc=0 已有活例并修正规则 v1.5）；计数器原子替换写入（三窗口钩子实证）。C5 为最小双链口径（nominal S1–S3），全量同会话双链仍为环境限制未覆盖组合，不宣称"验收自动化完全闭环/永不崩溃"。修复与关闭验证见 [`../evidence/M2_REACCEPT_ROUND8_FIX_20260902.md`](../evidence/M2_REACCEPT_ROUND8_FIX_20260902.md)，第八轮验收见 [`../evidence/M2_REACCEPT_ROUND8_CODEX_20260902.md`](../evidence/M2_REACCEPT_ROUND8_CODEX_20260902.md)；第七轮历史见 [`../evidence/M2_REACCEPT_ROUND7_CODEX_20260902.md`](../evidence/M2_REACCEPT_ROUND7_CODEX_20260902.md) 与 [`../evidence/M2_REACCEPT_ROUND7_FIX_20260902.md`](../evidence/M2_REACCEPT_ROUND7_FIX_20260902.md)。本文档先于实施落地，交付清单来源为 `docs/PROJECT_EXECUTION_ROADMAP.md` §5 M2。
