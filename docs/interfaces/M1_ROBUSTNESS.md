...
状态：**已完成并通过验收（2026-09-01）**，证据与数据见 [`../evidence/M1_ROBUSTNESS_20260901.md`](../evidence/M1_ROBUSTNESS_20260901.md)，独立复验见 [`../evidence/M1_REACCEPT_CODEX_20260901.md`](../evidence/M1_REACCEPT_CODEX_20260901.md)。来源：`docs/PROJECT_EXECUTION_ROADMAP.md` §5 M1 与 §6"下一次实际工作"。本文档是 M1 的执行基线：先落文档，再实施，实施完成后按 §7 判据验收并回填 §8。

项目组：周航正、霍奕茗、于跃、叶安、王健祺
文件负责人：叶安
主要撰写：叶安（场景矩阵与判据决策）、ZCode（方案文档代拟、注入脚本实现与验收执行）
技术贡献：Codex（M1 独立复验，意见 F1--F5 已落实）
审核：待项目组审核、待指导教师确认
AI协助：ZCode（内存注入脚本、探针与本文档代拟）
...
`M0A Power Measurement` 输出 1（`P_est` 真实值）经探针核实（`diag_m1_probe.m`）后共三路下游：`M0C P ZOH` 入 1（ESC 的 `P_e`）、`M0A P Est ZOH 1ms` 入 1（再喂 `M0A Constraint Flags` 入 4，功率安全监视）、`M0A Log P Est` 等日志分支。M1 的主注入操作是**重接前两个 ZOH 的输入**到退化链输出（M0-B 注入模式，日志分支不动）：

```
M0A Power Measurement 出1 ──┬── (原线保持) 日志 m0a_P_est_W 等分支（真实值）
                            ├── [时延链 z⁻¹⁰ (0.05 s×10), 初值 251] ──┐
                            └──────────────────────────────────────── + Sum ──┬── M0C P ZOH 入1
                            Random Number (σ, Seed) ──────────────────────────┘   └── M0A P Est ZOH 1ms 入1 ── flags 入4
```

- **ESC 与功率监视器看到一致的退化测量**（真实传感器退化语义），二者不因注入而信息不对称；
- **能量/成本统计保持真实功率口径**：`m0a_P_est_W` 日志留在注入点上游，regret、`Eclean` 等指标不被测量噪声污染；测量退化只体现在安全链行为与 ESC 决策中；
- 脚本运行时断言两条被重接线的原源都是 `M0A Power Measurement` 出 1（防拓扑漂移，探针 `diag_m1_probe.m` 可复核）；
...
F 组在**名义背景（roll 正弦置 0）**下重跑 M0-B 四类注入，判据保持 M0-B 原语义。名义背景是 M1 首轮实施后的修正：基线正弦扰动下位 5（速度失跟）在 pre 窗本有活动，与加严判据"pre 窗 8 位全静默"语义冲突；置 0 后该判据才准确表达"噪声不产生任何误触发"。判据 c1–c6 与 M0-B 仓库现行版逐条相同。

- F1/F2/F3（pwm_edge / yaw_rate / nan_power）：主 P 通路插噪声链（同 §2.1），故障注入照旧重接 `M0A Constraint Flags` 入 1 / 入 2——两个操作作用于不同输入端口，可叠加；
- F4（power_rec）：故障源（251 + 1349 脉冲 Sum）内嵌同一噪声 Sum，重接入 4；ESC 侧 P 保持原线（fallback 后 selector 主导参考，ESC 输入退化与否不影响安全判据，如实记录该口径）；
...
**全部判据通过**（27/27 场景，正式归档 `results/air_m1_robustness/20260901_160420/`，证据 [`../evidence/M1_ROBUSTNESS_20260901.md`](../evidence/M1_ROBUSTNESS_20260901.md)）：

1. §7.1：R0/WN1–5/DL 全部 esc 臂收敛（4–20 s）、带内运动，8 位约束标志/frozen/fallback 全程为零——2% 噪声与 0.5 s 时延零误触发；
2. §7.2：11 组配对 regret 最大 |0.000133%| ≪ 3%（平坦面预判再次成立，无可宣称节能）；
3. §7.3：DL2 与 DL1-esc 逐样本差 0；
4. §7.4：WD/CM 零安全动作、末段参考回到 9±0.51，clean 窗占比 0.39–0.46 如实报告；噪声/组合把收敛从 4 s 放慢到 8–20 s（记录在案，未调参掩盖）；
5. §7.5：F1–F4 全过——pre 窗 8 位静默、触发 0.000 s / frozen 0.000 s / fallback 0.500 s、位 6 恢复，与 M0-B 验收时序一致；
6. §7.6：平台线 5 脚本回归全绿；
7. §7.7 结论边界照抄 §1/§6。

实施修正记录（首轮归档 `20260901_155726` 不通过，详见证据 §2）：注入点拓扑经探针 `diag_m1_probe.m` 核实后改为重接 `M0C P ZOH` 与 `M0A P Est ZOH 1ms` 两个输入（监视器功率输入的上游 ZOH）；F 组背景改为名义（roll 置 0）以使"pre 窗 8 位静默"判据语义成立。判据门槛与种子未因结果调整。
