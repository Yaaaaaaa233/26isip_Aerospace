...
版本：v0.1（建议/Proposed，待 R0 组内确认后冻结为 1.0）；2026-09-21 增补 §10（四旋翼阶段：驻留语义 / P̂ 估计口径 / 双口径记账，Proposed 待 H5 审读）
日期：2026-09-05
...
## 10 四旋翼阶段增补（2026-09-21，Q3，Proposed 待 H5 审读）

依据 [`../QUAD_MIGRATION_PLAN_20260921.md`](../QUAD_MIGRATION_PLAN_20260921.md) §4-Q3。本节只增补不改写 §1–9；三条增补均为四旋翼迁移阶段（Q 系列）的合同细化。

### 10.1 指令驻留语义（t_dwell）

- 每条新 `v_ref` 指令生效后，系统需驻留 **≥ t_dwell** 再采样评价，测量面才代表新指令的就位状态。实测值：**t_dwell ≥ 2 s**（Q1/S5 驻留扫描：≤2 s 无害、5 s 侵蚀预算；Q2 轨 B 电机一阶动态 4τ = 0.32 s，覆盖充足；参数到位后按 4τ 复核）。
- **对外合同语义 = 驻留**：实机飞控没有"等就位再返回"的原语；平台链 `q()` 的就位委托制是仿真便利，委托制下驻留/就位时间照计评价预算（与实机口径一致）。
- 算法侧不得假设指令即时生效，也不得读取驻留期内的中间测量作为就位状态。

### 10.2 估计口径功率测量（P̂）

- 四旋翼阶段算法可见的功率量测为**估计器输出 P̂**，由指令侧 PWM + 传感器侧端电压合成（L0：静态台架查表；L1：L0 + 声明系数气速修正，修正空速由测量地速 + 风测量合成）。
- 因果红线（§4.2）不变：植物真值功率只供评价器；P̂ 的结构误差（台架静态映射不含前飞诱导修正的部分由 L1 补齐）与标定残差按可申报误差带限定（常数 ≤5%、倾斜 ≤1.4 %/m/s）。
- 指令后驻留期（§10.1）内 P̂ 不代表新就位点功率。

### 10.3 双口径记账

- 评价层对同一幕同时记录**真值口径**与**估计口径**（`powerTrue` / `powerMeas` 两列 + 差值），估计口径结论须带"估计口径"限定词发布；评价参照（minPower 滑动电压下界）保持真值口径——评价≠控制，参照可用真值。
- known 臂按 3.8 修复语义（真值相位死推）仍为 oracle 上界，不入双口径对比。
