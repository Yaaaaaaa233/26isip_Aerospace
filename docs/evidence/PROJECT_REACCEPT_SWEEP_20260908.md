...
### 2.1 【已修复并复验】P2 合成功率无 U 形左支（H3 未接入）——G-U 门转绿
...
- **修复（2026-09-08 叶安拍板"2.1 按推荐修复"，同日实施）**：动量理论（执行器盘）诱导修正缺省接入——每桨扣除 `s=T·(vi0−vi(v))`（`vi0=√(T/(2ρA))`，`vi(v)=√((v/2)²+T/(2ρA))−v/2`，A=π·1.016²/4），`h3_induced_gain=1.0`，v=0 时 s≡0（悬停严格保持台架值）；共轴下桨节省同乘 (1+δ)。形式登记于 06 v1.2 §3.1 H3 行，G-U 升正式门槛（T2 清单 v1.1）。
- **复验实测**（`run_p2_chain_check` 23/23 门 PASS）：悬停 488.7 W 严格不变；**v\*=5 m/s、P(v\*)=360.0 W、相对悬停降幅 26.1%**；敏感性 gain∈{0.7,1.3} → v\*∈[4,6] m/s、降幅∈[16.7%,35.5%]（报告值）；适配器稳态保真 0.26–0.49%（≤1% 门）；饱和负向/恒等式/eta 方向全部不回归；**T2 正式跑批解冻**（WP6 按 P2_T2_WORK_PLAN 推进，A3 重预登记与 E_pred/场景预登记冻结照旧为前置）。
