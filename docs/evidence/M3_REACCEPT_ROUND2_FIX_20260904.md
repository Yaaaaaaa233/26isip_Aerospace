...
# 勘误块（2026-09-04 追加，第三轮修复时随 M3_REACCEPT_ROUND3_FIX 同步入库）

本块按项目惯例追加于原文之后，原文一字不改；依据为第二轮独立验收报告
[`M3_REACCEPT_ROUND2_CODEX_20260904.md`](M3_REACCEPT_ROUND2_CODEX_20260904.md) §3.4 的纠偏，
全部属实，由本修复方复核确认：

1. **生产负向计数**：本报告第 1 节"生产路径负向 6 例 + 正控"有误。实际为
   **5 例负向 + 1 正控**（R2-N4/N5/N6/N7/N8 + R2-P0）。验证器逐例打印可复核。
2. **N5/R1 所在段 runId**：本报告第 3 节写作 `46d8c29c…`。实际归档
   `results/air_m3_trials/20260904_134706/result.mat` 的 binding.runId 为
   `b9573d34-9f88-497c-95a9-dcc6fbc53d8f`。
3. **"14 臂 chk.pass=1" 表述不成立**：`m3_eval_arm` 的执行 checker 仅对
   modeV/modeEta 含 'm3' 的臂运行，6 个非 M3 基线臂（B0×2/B1×2/B2×2）
   `chk=[]` 跳过。正确表述：**14 臂 r.ok=1；8 个 M3 臂 checker PASS**。
4. **恢复矩阵覆盖表述**：第二轮 11 行为**针对性矩阵**（四态 × 预写出口、
   {finite,NaN} × cfgmismatch、{finite,NaN} × 写入后出口等），不构成
   "四态 × 全部出口"覆盖；全局量注册表相应行不得外推为全覆盖。第三轮
   修复已将矩阵补齐为全笛卡尔 28 行（trials 4 态 × 5 出口、boundary
   4 态 × 2 出口），见第三轮修复报告。
5. **状态页一致性**："待确认后冻结 M3" 与既有冻结记录冲突，已由第二轮
   独立报告同步纠正。

**总体勘误声明**：本报告为修复方复验，不构成独立验收；第二轮独立验收
结论（验收基础设施 NOT VALIDATED，F2/F3/F4 三项 P1 开放）优先于本报告，
三项 P1 的关闭以第三轮修复报告与后续独立复验为准。
