...
更新时间：2026-09-09（第十七次更新：**3.6 platform_link 关闭**——人工验收 H1–H5 叶安确认通过（含样张图1画图勘误：polyval 降幂误读升幂系数，重出后拟合谷底 5.48，算法与门槛结论不受影响）。第十六次更新：3.6 接手修复完成——叶安按方案 [`T36_PLATFORM_LINK_TAKEOVER_PLAN`](T36_PLATFORM_LINK_TAKEOVER_PLAN.md)（D1–D3 拍板）修复王健祺 WIP 移交的平台对接：根因 F1 = `q()` 误传内环步数使每次查询推进 100 s（标定从未执行，û\* 停在初值 7.5）+ F2 Phase B 风修正无界发散 + F4 无种子 + F5 R2022b 环境探测；三道守卫落 `sweepcal_run`；正式批 52/52 单测 + 20/20 门槛两遍逐位确定，平台五门 û\*=5.0765/尾段 2.98%/purerl_off 4.71% vs 开环 5.61%；证据重出并归档旧件，模块登记补全（治理 PASS）。第十五次更新：T2.4 验收关闭，P2/T2 平台线交付全部完成；第十四次：T2.4 eta 特性展示正式批落地；第十三次：T2.4 重定义 + 寻优平台冻结；此前：M3 放行/T2 关闭/H3 修复/全项目重验收/P2 落地/M3 §7.3）
