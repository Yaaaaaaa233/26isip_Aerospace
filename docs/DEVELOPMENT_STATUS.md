...
[`decisions/ADR-002-rl-readiness.md`](decisions/ADR-002-rl-readiness.md) 已记录当前决策：正式RL开发暂缓，现有 `speed_rl_residual` 仅保留为接口预研和候选插件；在任务口径、功率对象、统一接口、Harness、强基线及未见场景门槛成立前，不扩大训练或作“RL优于基线”结论。该架构文档不改变平台唯一执行路线的当前顺序。M2 的 120 s / `[90,120] s` 修订协议经多轮复验；第三轮发现的脚本异常退出恢复缺陷已按第四轮关闭验证修复（三入口函数化 + 真实错误注入矩阵 10/10 PASS，[`evidence/M2_REACCEPT_ROUND3_FIX_20260902.md`](evidence/M2_REACCEPT_ROUND3_FIX_20260902.md)），并同轮落地验收自动化规则（[`ACCEPTANCE_AUTOMATION_RULES.md`](ACCEPTANCE_AUTOMATION_RULES.md) v1.0）。M2 放行，进入 M3。
...
飞控平台的唯一执行基线是 [`PROJECT_EXECUTION_ROADMAP.md`](PROJECT_EXECUTION_ROADMAP.md)。M0-B 复核缺陷已修复并通过独立再验收（[`evidence/M0B_RERUN_20260901.md`](evidence/M0B_RERUN_20260901.md)、[`evidence/M0B_REACCEPT_CODEX_20260901.md`](evidence/M0B_REACCEPT_CODEX_20260901.md)）。**M0-C 已通过验收（[`evidence/M0C_TRIALS_20260901.md`](evidence/M0C_TRIALS_20260901.md)），M1 已通过验收（[`evidence/M1_ROBUSTNESS_20260901.md`](evidence/M1_ROBUSTNESS_20260901.md)）。M2 的受约束分配器、ESC 接线和修订后数值门槛通过；第三轮复验发现的脚本异常退出恢复问题已修复并通过第四轮关闭验证（[`evidence/M2_REACCEPT_ROUND3_FIX_20260902.md`](evidence/M2_REACCEPT_ROUND3_FIX_20260902.md)），验收自动化规则同轮固化（[`ACCEPTANCE_AUTOMATION_RULES.md`](../docs/ACCEPTANCE_AUTOMATION_RULES.md) v1.0）。当前进入 M3：速度与转速比交替协同优化（方案文档先行，`.slx` 结构集成照红线执行）**；不做 RL。
...
1. M3（速度与转速比交替协同优化）：两个 ESC 的更新/保持/优先级仲裁、同一条稳定窗口与约束总线、与 M0/M2 单变量基线同场景比较；方案文档先行，`.slx` 结构集成照红线与 M2 实现教训执行。M2 第三轮自动化问题已关闭（2026-09-02，第四轮 10/10 PASS）。
