...
更新时间：2026-09-21（第二十三次更新：**Q4 正式验收批 PASS、Q 系机器验收（M1–M8）全部闭环**——[`platform_link_39`](../modules/platform_link_39/) 按 [`QUAD_MIGRATION_PLAN_20260921.md`](QUAD_MIGRATION_PLAN_20260921.md) §4-Q4 执行：9000s 正式批 ×2（l1 估计链五臂 + truth 真值基线五臂，同种子，16 独立 MATLAB 会话全 rc=0，挂钟约 4h20m，预注册 `q4_opt.m` 冻结）。**学习臂全部优于开环**（真值口径超额 sweepcal 2.0823% / purerl_off 1.0717% / purerl_on 0.6230% vs 开环 6.3418%；赢面 +4.26/+5.27/+5.72pp）；**与同种子真值口径基线比的赢面损耗 −0.063/+0.218/+0.132pp（门 ≤1pp）**；×2 逐位确定（修正门定义后仿真派生量无一失配）；双口径结论漂移 ≤0.026pp；openloop/known 真值口径跨批逐位相等（植物冻结 9000s 复验）；rowsN 9000~9006（契约内）。**M6/M7 门 6/6 PASS**；M8 已在 Q3 批存档（52/52+20/20）。门定义预注册修正三项先行登记后复评（runtime_s/锚点臂 margin 列逐出逐位集、G6 补 estMode 过滤——首评 FAIL 存档与终评 PASS 存档均在案，批数据零改动）。剩余人工门 H1–H6（叶安；H1 演示依赖 3600s UI 修复入库、H5 审读接口 §10）与外部确认 C1/C2/C3 维持非阻塞。第二十二次更新：**Q3 估计链接入完成、P5 冻结回归不动**——[`platform_link_39`](../modules/platform_link_39/) 交付 **L1 估计器**（D1 执行口径：L0 静态台架映射 + 声明系数气速修正 dH3/dH5/dDrag/aux，与 trim 分解逐项同型；`decl` 覆盖参数留 Q4 降档敏感性；估计空速 û̂=|(测量地速−风测量)·切向| 只用测量面字段，因果结构门+文本门双保险）、`estMode='l1'` 测量链分支（powerMeas ← P̂，0.2s 时延+1.2% 噪声照搬 T1，真值列逐位不变）、**M7 双口径记账层**（每臂 true/est/diff 三行；known 仍按 3.8 语义为 oracle 上界）。单元门 8/8（L1 自洽 1e-12、悬停域 L1=L0+aux 精确、配平域闭合 ≤1e-9、植物级 l1 尾偏差 ≤2% vs l0 ≈7%、M7 恒等式、两遍逐位、因果门）。**冒烟 3600s×4 臂（l1 口径）全 rc=0：sweepcal 赢面从 L0 的 −11.65pp 崩溃恢复到 +2.34pp**（真值链锚点 +2.24pp 同水平）；purerl_off/on 赢面 +5.42/+5.67pp（vs 真值锚点磨损 ≤0.39pp < M6 门）；openloop 真值超额与 Q1 ANCHOR 逐位一致（植物零改动直接证据）；**双口径结论漂移 ≤0.029pp（超额）/≤0.041pp（赢面）** ≪ M6 门 1pp——估计口径评价结论与真值口径一致，**Q4 正式批按 D1 在 L1 口径执行**（m7_rows 差值行机制直接复用）。接口文档 `SIM_ALGO_INTERFACE.md` 增补 §10（t_dwell ≥2s 驻留语义"对外合同=驻留"、P̂ 估计口径语义、双口径记账），Proposed 待 H5 审读。X8 冻结回归 **52/52+20/20 不动**（P5/M8；重生成 CSV 与 F5 时代归档的差异逐项归因于 F5 后已入档的合法模块变更与拟合臂 BLAS 末位噪声，openloop 行逐位一致）。调试勘误：û̂ 合成初版符号写反（+风测量，违反接口 §6 减号约定）被接线门当场拦截。第二十一次更新：**Q2 双轨完成、M4' 对拍门通过**——[`models/plane_quad`](../models/plane_quad/) 轨 B `+quad_sim` 交付：x8phys 结构（油门正向/KV·V 耦合/一阶电机滞后/四元数刚体）+ MN1005 台架标定（T(n)/P(n;V)/H3 替代代理系数）+ 自带飞控（H6 型内环，传感器集=实机 FC 可观测量）；电气记账与轨 A 同口径（调试中发现并修复两项结构错：垂直通道符号 az_fb=+kvz·vz、废阻功须计电功率——x8phys 原口径高速端差 33%）。**M4'：6 配平点 0.009–0.36%（门 ≤1.5%）、就位比 0.92（B 10.75s/A 11.70s，门 ≤2×）**，轨 B 单元门 7/7。**n 效应复核（功率侧）**：Jgain=3 下 P̂_L0 全程平坦（极差 <1%）——L0 盲性在独立结构复现，D1 升 L1 加固；bias 驼峰 +19.9%（谷区）→−9.5%（高速）；J² 把真谷底 5.65→5.00 m/s；推力侧 n 效应（pwm 携带空速信息的唯一通路）无数据待 C2。**t_dwell 定稿 ≥2 s**（电机 4τ=0.32 s 覆盖充足）。B2 Simulink 薄包装列后续。第二十次更新：**Q2 轨 A 交付**——四旋翼降阶链（单元门 M2/M3 8/8，悬停 615 W、谷底 5.65 m/s、H9 裕度 20.8%）；同批勘误 Q1 谷深 7.6%→23.5%。第十九次更新：**Q1 估计器敏感性批完成**——L0 静态台架估计器 + S1–S5 矩阵 19 块全 rc=0（M5 逐位 PASS、ANCHOR 复现 moe38）；L0 纯静态映射 P2 口径下不可部署（谷由 H3 创造、sweepcal −11.65pp）；b₁\*=−2.6~−2.8 %/m/s、purerl ±2.8 未破；常数偏差无害；D3 电压归一化不需要；D1 升 L1 档；误差带草案"常数 ≤5%+倾斜 ≤1.4%/m/s→磨损 ≤0.9pp"。第十八次更新：**Q 系阶段立项**（老师约束实机只能观测油门+电压、载具换四旋翼；方案 QUAD_MIGRATION_PLAN，Q0–Q4/M1–M8/H1–H6）+ 补记算法线 9-11~9-18（N9/N10、T3.7、T3.8）。第十七次：3.6 关闭。第十六次：3.6 接手修复完成（F1–F5，52/52+20/20）。第十五次：T2.4 关闭、P2/T2 平台线交付完成；第十四次：T2.4 正式批；第十三次：T2.4 重定义；此前：M3 放行/T2 关闭/H3 修复/全项目重验收/P2 落地/M3 §7.3）

项目组：周航正、霍奕茗、于跃、叶安、王健祺
文件负责人：叶安
本次修订：周航正（提出名义功率图、在线寻优、接口草图与RL位置澄清需求）；王健祺（2026-09-03 新增 realistic_constraints_search / curve_case_calibration / wind_model_library 三模块登记与进度补充；2026-09-05 新增 wind_semantics_correction / wind_inference_search 两模块登记、MOE口径变更（纯能耗）与本地任务线 1.x/2.x/3.x 重命名；2026-09-06 新增 curve_unknown_search / reward_only_rl 两模块登记并对接 SIM_ALGO_INTERFACE；2026-09-07 三模块算法集精简（删除 task1 遗留直接搜索器，见 worklog 2026-09-07）+ demo 风场预设修复 + turb σy 修正，回归全绿并刷新证据，新增 docs/VERSION_REVIEW_20260907_wjq.md 全版本梳理；2026-09-08 新增 unified_four_algos / curve_calib_windinfer 两模块登记（3.4四算法整合横比、3.3标定+风推断混合），修复截断拟合边缘假谷（样本支撑谷底选择+标定块支撑+û*锚定信赖域+删半程拟合+RL探索域钳位），盲测谷底恢复4.5–9.5，回归全绿（3.4 42/42+13/13、3.3 37/37+11/11，见 worklog 2026-09-08）；2026-09-08 新增 coaxial_ratio_opt 模块登记（工作包w11：固定速度下共轴桨比 r=η_Ω 在线功率寻优——Opazo 2022 MT链真值+γ等效诱导干扰因子场，四主角 sweepcal/rl/purerl/hybrid 移植+oracle 参照，43单测+15门槛全绿，见 worklog 2026-09-08））；Codex（X8PHYS/P0--P4 审核、验收收口与结论边界，2026-09-03）；叶安（2026-09-07 拍板：Plane 线并入平台线、ENV 维持王健祺口径、整机 10 kg/电池 7S4P、缺省假设着重标注登记、eta 接口保留恒 1、每阶段前置可视化样张规则、暂只停留在模拟层面；2026-09-08 拍板：P2 推力回馈与限幅分层（06 v1.1）、电芯报告人工取数单硬规则、确认 [T1 验收标准清单](T1_ACCEPTANCE_CHECKLIST.md)含 E_pred 共用分母口径、T1 验收关闭、5 点 xlsx 方案 2 处置、**P2 前置六项按推荐答案定案并确认 [P2 工作计划](P2_T2_WORK_PLAN.md)与 [T2 验收标准清单](T2_ACCEPTANCE_CHECKLIST.md)（bit4 饱和位 / H9 涌现上限 / H5 缺省 0.90 / proxy 退役 / A3 重预登记 / capScale 仅样张，06 v1.2）**）

审核：待项目组审核
AI协助：Codex（路线与状态整理；M2 第六轮、第九轮与第十轮独立验收结果回填；2026-09-03问题与接口修订；2026-09-04 按叶安要求回填 M3 第二轮独立验收，纠正提前关闭表述；2026-09-05 第三轮独立验收回填，区分机制修复、证据链缺口与环境崩溃）；ZCode（2026-09-05 按接口研讨会登记两阶段验收口径与接口/路线文档；2026-09-07 并线登记、仓库整合与平台动力-环境模型方案成文；2026-09-08 T1 前置样张生成与 21 项指标核验、验收清单与 P1 方案成文入库；2026-09-08 T1 执行日：M3 F4 修复+自验、§7.3 正式批次复验 PASS、适配器 5 门、台架清洗+拟合、电池模型 v1、P2 前置样张与协议位表核对、P2 方案与 T2 清单成文，见 worklog 2026-09-08）
...
| PX4/Simulink Control 平台 | M2 已放行；**M3 已放行**（第四轮独立复验 C1–C7 PASS，2026-09-08，限 V1 px4_x8 代理阶段 @3eedfa8） | [第三轮独立报告](evidence/M3_REACCEPT_ROUND3_CODEX_20260905.md) 四项已实证缺陷（§3.1–§3.4）全部修复并机器验证（[修复证据](evidence/M3_F4_FIX_20260908.md)）：严格 live 绑定+7 指纹、正式入口三方记账、归档配置+14 臂生产复核、验证器合同门与记账审计；非仿真套件 25 内层负向+12 正式入口探针全绿；§7.3 正式复验 11 阶段 attempt-1 PASS（14 臂复核 replayDiff 全 0、配对数值与第三轮一致、恢复矩阵 28 行、回归全绿） | R2022b 堆崩溃仍为环境 OPEN LIMITATION（本轮未发生）；M3 未独立复验前不放行；不外推统一 Plane/R4/实飞 |
...
| 算法线第二轮 2.x（风速推断寻优） | 验收入口全绿（27测试/8门槛） | 恒定风下零探针策略逼近 known 上限（MOE 0.9945 vs 0.9988），突破第一轮"谷底测量不含风信息"的探针瓶颈（激励改为转圈航向扫描，问题从盲搜移动最优变为估2个风参数+闭式解算）；变风仍受滑窗滞后约束（稳态RMSE 2.17 m/s）；因果性经假config试验等三重核验；依赖曲线先验，2.2 曲线未知后需换强化学习路线（待用户确认后启动） |
...
| `modules/wind_model_library` | 2026-09-03 并入（本地工作名 task9）。七种可选风场（const/sin/软边方波/三角/OU湍流/复合/扇区）+选中即预览+空速地速语义显式化+τ=0对照口径；18 测试、5 门槛 | 空速恒等式 1e-9；扇区风下 qnewton 唯一明显胜开环（4.65-5.92% vs 4.86-6.70%）；τ=0 与 τ=0.3 几乎相同（时延非瓶颈）；归因：MOE下降主因是风→最优值摆幅较任务6放大约7-8倍 | ⚠️参数未实测校准、加号约定待适配（适配参考见 wind_semantics_correction）；相位查表/风感知前馈已由 wind_inference_search 以"功率调制风推断"路线部分实现；真实 X8 节能 |
| `modules/wind_semantics_correction` | 2026-09-05 并入（本地编号 1.11，原 task10）。风速语义修正参考实现：空速=地速−风（接口字典0.3口径）、风不影响运动、空速-功率曲线固定；继承实际约束/曲线case/七风场；19 测试、5 门槛 | 空速恒等式与闭式最优 1e-9；用户顺风/逆风例子通过；τ=0 对照通过；3×3 验证表入库 | ⚠️曲线/风参数未实测校准；前序模块加号约定尚未批量适配；真实 X8 节能 |
| `modules/wind_inference_search` | 2026-09-05 并入（第二轮 2.1）。风速推断寻优：u\* 固定已知、风未知，转圈功率调制反演风矢量（滑窗NLS+自适应窗+风况判定）+闭式地速调度，全程零探针；27 测试、8 门槛 | 恒定风 MOE 0.9945 vs openloop 0.9399、known 0.9988（吃到上限增益约92%）；因果性三重核验（假config试验：距真风0.09/距假风10.68 m/s）；恒定风估计误差均值0.11 m/s、约0.4圈收敛 | ⚠️依赖曲线先验（2.2曲线未知后不直接适用）；变风受滑窗滞后限制（稳态RMSE 2.17 m/s）；真实 X8 节能 |
| `modules/curve_unknown_search` | 2026-09-06 并入（本地编号3.1，用户称task3）。曲线未知在线寻优：全速度域双向快扫(3→12→3,150步)联合辨识曲线f与风w + 闭式调度 + 探针精化(sweepcal)；RL仿真器预训练+在线微调为对照；33 测试、8 门槛 | 恒定风 sweepcal 1.91%（开环6.40%、known 0.11%；稳态尾段0.37%）；变风 RL 3.52% 优于 sweepcal 5.10%（追漂移更快）；单向上扫样本不可辨识(û*偏差5.8)→双向扫0.08 | ⚠️依赖扫频信息、变风追踪滞后；曲线/风参数未实测；真实 X8 节能 |
| `modules/reward_only_rl` | 2026-09-06 并入（本地编号3.2）。纯奖励RL（无扫频/无模型/免标定学费）：大幅值对偶探索(±1.2→±0.5)+邻域共享核更新+分航向桶基线+表格型actor(12桶)直接从奖励学每航向最优地速；37 测试、9 门槛 | 恒定风 3.63% vs 开环 6.40%；变风 3.51% vs 开环 4.72%；免150步标定学费；稳态精度低于标定法(1.60%)为二阶奖励+1%噪声的信息论代价（诚实标注） | ⚠️探索幅值本身有功率代价(≈0.7%/步退火后收回)；纯在线无预训练版≈开环；真实 X8 节能 |
...
4. **并行算法/Control线**：叶安按 [M3 v0.2](interfaces/M3_V_ETA_COORDINATION.md) 与[第三轮独立报告 §7](evidence/M3_REACCEPT_ROUND3_CODEX_20260905.md) 推进。F2/F3 已在问题范围关闭；F4 已修复并完成 §7.3 正式批次复验（11 阶段 attempt-1 PASS @ `d552a14`，见 [M3 F4 修复证据](evidence/M3_F4_FIX_20260908.md) §5）。M3 已放行（第四轮 C1–C7）；T2.4 已按重定义口径完成 eta 特性展示正式批（E1–E6 全 PASS，见 [T24 证据](evidence/T24_ETA_CHAR_20260908/README.md)），原 M3 十四臂统一 Plane 复跑已登记性取消、寻优平台冻结。
