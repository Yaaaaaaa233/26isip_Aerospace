...
更新时间：2026-09-08（第十三次更新：**T2.4 重定义 + eta 寻优平台冻结**（叶安拍板"eta 特性展示"三决策）——原 M3 统一 Plane 复跑登记性取消；eta-P 模型适度增密：δ(v)=δ₀·(vi/vi₀)^κ（κ=0.5 缺省）、删除人造回中项、谷底 η\*(v) 成模型输出（悬停≈0.90 文献锚→巡航≈0.94 回漂）；[T24_ETA_CHAR_PLAN](T24_ETA_CHAR_PLAN.md) v1.0 + 06 v1.3 + T2 清单 v1.3（门槛 E1–E6 预登记）；实施待启动。第十二次更新：M3 放行（Codex 第四轮 C1–C7 全 PASS）；此前：T2 关闭/T2 27×27/H3 修复/全项目重验收/P2 落地与前置交付/M3 §7.3）
