...
[`decisions/ADR-002-rl-readiness.md`](decisions/ADR-002-rl-readiness.md) 的平台决策仍有效：虽然 `speed_rl_pytorch` 已完成 TD3/BC 独立训练与评估，但正式 RL 平台接入仍暂缓；在统一 Plane 对象、Harness、强基线及未见场景门槛成立前，不作“RL优于基线”结论。M2 的 120 s / `[90,120] s` 核心数值协议保持放行并进入 M3；第八轮发现（R8-F1/F2/F3）已修复并关闭：attempt 证据硬断言、入库有界重试驱动、原子标记，48/48 针对性矩阵（含正式批次内受控击杀后的自动重试），验收自动化在针对性矩阵 + 入库驱动口径下成立。同时按 [`PROJECT_EXECUTION_ROADMAP.md`](PROJECT_EXECUTION_ROADMAP.md) 推进 P0--P4 Plane 物理建模并行工作线。
...
| `models/px4_x8` | 阶段 0、M0-A、M0-B、M0-C、M1、M2 核心已放行；第八轮发现已修复，当前提交 48/48 针对性矩阵（R8-F1/F2/F3） | 干净与脏入口下旁路与原基线零差异；M0-B 安全注入 4/4；M2 S1/S2/S3 = −0.2626%/−0.2938%/−0.2147%；源码指纹与 verdict 硬断言落地；attempt 证据硬断言（含六类负向证明）+ 入库有界重试驱动 + 原子标记；48 行矩阵绑定 runId `72300de6` @ `6f6672c`，c5 attempt=2（正式批次内受控击杀后自动重试），双链哈希一致 | 不外推真实功率/风场；R2022b 堆崩溃与启动器退出码不可靠仍是环境缺陷（本轮再增两个真实样本）；全量同会话双链未覆盖；HEAD 前进需整批重跑 |
...
飞控平台的唯一执行基线是 [`PROJECT_EXECUTION_ROADMAP.md`](PROJECT_EXECUTION_ROADMAP.md)。M0-B 复核缺陷已修复并通过独立再验收（[`evidence/M0B_RERUN_20260901.md`](evidence/M0B_RERUN_20260901.md)、[`evidence/M0B_REACCEPT_CODEX_20260901.md`](evidence/M0B_REACCEPT_CODEX_20260901.md)）。**M0-C、M1 已通过；M2 受约束分配器、ESC 接线和修订数值门槛保持放行。第八轮发现（R8-F1 attempt 证据未硬断言、R8-F2 重试驱动未入库、R8-F3 计数器非原子写入）已于同日修复并关闭：report 硬拒绝六类非法/不一致 attempt 证据（负向矩阵 6 行入矩阵）、`tools/run_m2_batch.ps1` 入库（新鲜完成证据权威、退出码仅参考——修复期间捕获 matlab 启动器对堆崩溃子进程报 rc=0 的活例并同步修正规则 v1.5）、计数器原子替换写入；48/48 针对性矩阵 PASS，runId `72300de6` @ `6f6672c`，c5 以第 2 次尝试通过（正式批次内受控击杀→自动完整重执行→`done.attempts=2` 如实记账）（[`evidence/M2_REACCEPT_ROUND8_CODEX_20260902.md`](evidence/M2_REACCEPT_ROUND8_CODEX_20260902.md)、[`evidence/M2_REACCEPT_ROUND8_FIX_20260902.md`](evidence/M2_REACCEPT_ROUND8_FIX_20260902.md)）。验收自动化在针对性矩阵 + 入库驱动口径下成立，不宣称完全闭环；环境缺陷未消灭，全量同会话双链仍为未覆盖组合。M3 方案与实现可继续；不做平台侧 RL。**
