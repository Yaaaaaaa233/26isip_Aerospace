...
| `models/px4_x8` | 阶段 0、M0-A、M0-B、M0-C、M1、M2 核心已放行；第六轮发现已修复（42/42 针对性矩阵，C5 为最小双链口径） | 干净与脏入口下旁路与原基线零差异；M0-B 安全注入 4/4；M2 S1/S2/S3 = −0.2626%/−0.2938%/−0.2147%；stage 源码指纹在 init/段开始/盖章/report 独立现场取证（拒 unknown/脏树/不一致），聚合器逐行 verdict 硬断言；42 行矩阵绑定 runId `ff8636ec` @ `2d36288`，双链哈希一致 | 不外推真实功率/风场；esc 中心受 PWM 量化限制；全量 9 场景同会话双链为环境限制未覆盖组合（R2022b 堆损坏，最小双链替代）；report 绑定批次提交，HEAD 前进需整批重跑 |
...
飞控平台的唯一执行基线是 [`PROJECT_EXECUTION_ROADMAP.md`](PROJECT_EXECUTION_ROADMAP.md)。M0-B 复核缺陷已修复并通过独立再验收（[`evidence/M0B_RERUN_20260901.md`](evidence/M0B_RERUN_20260901.md)、[`evidence/M0B_REACCEPT_CODEX_20260901.md`](evidence/M0B_REACCEPT_CODEX_20260901.md)）。**M0-C、M1 已通过；M2 受约束分配器、ESC 接线和修订数值门槛保持放行。第六轮三项发现（R6-F1 源码指纹自引用、R6-F2 聚合 verdict 未硬断言、R6-F3 C5 环境阻塞）已于同日修复并经 42/42 针对性矩阵关闭验证（runId `ff8636ec` @ `2d36288`，C5 最小双链口径；[`evidence/M2_REACCEPT_ROUND6_CODEX_20260902.md`](evidence/M2_REACCEPT_ROUND6_CODEX_20260902.md)、[`evidence/M2_REACCEPT_ROUND6_FIX_20260902.md`](evidence/M2_REACCEPT_ROUND6_FIX_20260902.md)）；全量同会话双链仍为环境限制未覆盖组合，不宣称"验收自动化完全闭环"。M3 方案与实现可继续；不做平台侧 RL。**
