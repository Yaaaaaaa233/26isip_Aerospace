...
更新时间：2026-09-21（第二十次更新：**Q2 轨 A 交付**——[`models/plane_quad`](../models/plane_quad/) 四旋翼降阶链落地：`plane.step`（P2 冻结）骨架复用、4 电机单桨、eta 恒 1（接口字段保留），H3/H4/H9/电池链与 P2 同式、标定从冻结源只读、`quad_placeholder` 占位口径（10 kg 承 X8、MN1005/7S4P）。单元门 **M2/M3 8/8**（悬停恒等式/能量积分/功率平衡 ≤1e-12、H9 触发+封顶恒等式、截止行为；eta/共轴清除静态扫描+行为锚点）。轨 A 锚点：悬停 615 W、稳态谷底 5.65 m/s @ 469.7 W（谷深 23.6%）、k=9.91 W/(m/s)²、悬停 H9 裕度 20.8%（10 kg 占位可行）；M4' 轨 A 侧配平基线（5 V×8 配平点）落档，低电压档为封顶运行明示。轨 B（Simulink 油门正向）与 M4' 对拍门待后续。同批勘误：Q1 证据文档谷深 7.6%→23.5%（按归档 CSV 实测值更正，结论不变）。第十九次更新：**Q1 估计器敏感性批完成**——[`platform_link_39`](../modules/platform_link_39/) 落地 L0 静态台架估计器（pwm+V→P̂，38号链副本基底，植物物理层零改动双口径）并跑完预注册 S1–S5 矩阵（19 块独立会话 3600s×seed11 全 rc=0，单元门 8/8，M5 两遍逐位一致 PASS，ANCHOR 逐位复现 moe38 锚点）。核心结论：**L0 纯静态映射在 P2 口径下不可部署**（平台曲线谷由 H3 前飞诱导项创造、L0 曲线无谷：sweepcal 赢面 −11.65pp、purerl_off 5.80→2.74pp）；临界倾斜 **b₁\*=−2.6~−2.8 %/m/s**（sweepcal 负向先破），purerl ±2.8 全档未破（R1 局部结构鲁棒猜想实测成立）；常数偏差 ±5% 无害；**D3 电压归一化不需要**（±1% 电压偏差→~±3% 常数平移，margin 零磨损）；驻留 ≤2s 无损、实机 t_dwell 留 Q2 轨 B；**D1 判定升 L1 档（气速修正）为四旋翼默认设计目标**，Q4 正式批不得在 L0 口径跑。可申报误差带草案：常数 ≤5% + 倾斜 ≤1.4 %/m/s 内五臂磨损 ≤0.9pp（< M6 门 1pp）。预注册修正两则入档：S1/S2/S5 移 truth 口径+合成残差（trim_curve 实测谷由 H3 创造、L0 无谷）；M1 门 1e-9→5e-5（P2 pwm 派生用一步延迟电压）。解释边界：P2 未建实机螺旋桨 n 效应（前飞转速下降），L0"灾难性"结论为保守上界，Q2 轨 B 复核。证据：[quad_migration/q1_sensitivity](evidence/quad_migration/q1_sensitivity/Q1_SENSITIVITY.md)。第十八次更新：**Q 系阶段立项**——老师约束实机功率不可直接测量（可观测=每电机油门+电池电压）且载具换为四轴四旋翼，叶安拍板 [四旋翼迁移与油门-电压功率估计链方案](QUAD_MIGRATION_PLAN_20260921.md)（Q0–Q4、机器门 M1–M8、人工门 H1–H6；算法接口不变，油门+电压→P̂ 估计放测量适配层，双口径诚实，占位参数不可引用）；同次补记算法线 9-11~9-18 进展：T3.6 N9 ψ̂ 冻结根因修复（count() 浮点漂移致 for 零迭代）与 N10 塌缩守卫统一、T3.7 已标定好变体（标定移独立试飞不计预算，900s 窗学费 10%+→0.27–2.33%）、T3.8 平台评价参照修复（minPowerTrue 随端电压滑动真下界，消除 26–90% 违约假象与 −1.21% 假超额，purerl_off 9000s 诚实值 +0.46% 仍最优；known oracle 相位死推修复 0.9167→0.9958），登记表同步补齐 platform_link_37/38。第十七次更新：**3.6 platform_link 关闭**——人工验收 H1–H5 叶安确认通过（含样张图1画图勘误：polyval 降幂误读升幂系数，重出后拟合谷底 5.48，算法与门槛结论不受影响）。第十六次更新：3.6 接手修复完成——叶安按方案 [`T36_PLATFORM_LINK_TAKEOVER_PLAN`](T36_PLATFORM_LINK_TAKEOVER_PLAN.md)（D1–D3 拍板）修复王健祺 WIP 移交的平台对接：根因 F1 = `q()` 误传内环步数使每次查询推进 100 s（标定从未执行，û\* 停在初值 7.5）+ F2 Phase B 风修正无界发散 + F4 无种子 + F5 R2022b 环境探测；三道守卫落 `sweepcal_run`；正式批 52/52 单测 + 20/20 门槛两遍逐位确定，平台五门 û\*=5.0765/尾段 2.98%/purerl_off 4.71% vs 开环 5.61%；证据重出并归档旧件，模块登记补全（治理 PASS）。第十五次更新：T2.4 验收关闭，P2/T2 平台线交付全部完成；第十四次：T2.4 eta 特性展示正式批落地；第十三次：T2.4 重定义 + 寻优平台冻结；此前：M3 放行/T2 关闭/H3 修复/全项目重验收/P2 落地/M3 §7.3）
